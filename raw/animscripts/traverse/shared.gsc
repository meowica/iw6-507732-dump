...
dog_handle_traverse_notetracks( note )
{
	assert( IsDefined( self.traverseAnim ) );

	bDoTeleport = false;
	if ( note == "gravity on" )
		bDoTeleport = true;
	else if ( note == "traverse_jump_start" && !AnimHasNotetrack( self.traverseAnim, "gravity on" ) )
		bDoTeleport = true;
	...
	if ( bDoTeleport )
	{
		...
		curNotetracks = GetNotetrackTimes( self.traverseAnim, note );
		startTime = curNotetracks[0];

		animDelta = GetMoveDelta( self.traverseAnim, 0, curNotetracks[0] );
		animCurHeight = animDelta[2];

		jumpEndNotetracks = GetNotetrackTimes( self.traverseAnim, "traverse_jump_end" );
		if ( jumpEndNotetracks.size > 0 )
		{
			animDelta = GetMoveDelta( self.traverseAnim, 0, jumpEndNotetracks[0] );
			animEndHeight = animDelta[2];
			animDelta = GetMoveDelta( self.traverseAnim, 0, 1 );
			stepOutHeight = animDelta[2] - animEndHeight;

			endTime = jumpEndNotetracks[0];
		}
		else
		{
			animDelta = GetMoveDelta( self.traverseAnim, 0, 1 );
		...
		animLength = GetAnimLength( self.traverseAnim );
		nFrames = int( (endTime - startTime) * animLength * 30 );
		assert( nFrames > 1 );
		nLerpFrames = max( 1, nFrames - 2 );

		animHeight = animEndHeight - animCurHeight;

		actualHeight = endNode.origin[2] - self.origin[2] - stepOutHeight;

		self thread teleportThreadEx( actualHeight - animHeight, 0, nLerpFrames );

		return true;
	}
}

dog_wall_and_window_hop( traverseName, height, skip_teleport )
{
	self endon( "killanimscript" );
	self traverseMode( "nogravity" );
	self traverseMode( "noclip" );
	...
	if(!isdefined( skip_teleport ) )
	{
		realHeight = startnode.traverse_height - startnode.origin[ 2 ];
		self thread teleportThread( realHeight - height );
	}

	self.traverseAnim = anim.dogTraverseAnims[ traverseName ];
...
dog_jump_down( frames, rate, traversal_anim, bNotetrackTeleport )
{
	self endon( "killanimscript" );
	self traverseMode( "noclip" );
	...
	if( !isdefined( traversal_anim ) )
		traversal_anim = "jump_down_40";
	
	self.traverseAnim = anim.dogTraverseAnims[ traversal_anim ];
	self.traverseAnimRoot = %body;

	if ( !IsDefined( bNotetrackTeleport ) )
		bNotetrackTeleport = false;
	
	if ( !bNotetrackTeleport )
	{
...
dog_jump_up( height, frames, traversal_anim, bNotetrackTeleport )
{
	self endon( "killanimscript" );
	self traverseMode( "noclip" );
...
dog_long_jump( traverseName, height )
{
	...
	self clearanim( %body, 0.2 );
	self setflaggedanimknoballrestart( "dog_traverse", anim.dogTraverseAnims[ traverseName ], 1, 0.2, 1 );
