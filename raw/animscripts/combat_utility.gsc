...
setupAim( transTime )
{
	...
	self.facialIdx = self animscripts\face::PlayFacialAnim( undefined, "aim", self.facialIdx );
}
...
aimIdleThread( lean )
{
	...
	lastIdleAnim = %add_idle;	// init to whatever.
	...
	for ( i = 0; ; i++ )
	{
		...
		else if ( animArrayAnyExist( "exposed_idle" ) )
		...
		// if it was the same anim, then just restart the anim, but don't restart the siblings, e.g. additive aims.
		if ( idleanim == lastIdleAnim )
			self setFlaggedAnimLimitedRestart( flagname, idleanim, 1, 0.2 );
		else
...
FireUntilOutOfAmmo_WaitTillEnded()
{
	...
	fireThread = thisthread;
	while ( true )
	{
		waittillframeend;
		if ( !IsDefined( fireThread ) )
		{
			self shootStopSound();
			return;
		}
...
shootAtShootEntOrPos()
{
	...
	else
	{
		...
		self shootPosWrapper( self.shootPos, false );
	}
...
GetPitchToOrgFromOrg( toOrg, fromOrg )
{
	if ( !isdefined( toOrg ) || !isDefined( fromOrg ))
	{
		return 0;
	}

	// If self is a spaceAI, account for roll
	if( isDefined( self ) && self isSpaceAI())
	{
		forward = AnglesToForward( self.angles );

		// Move pos to origin, and rotate it around self by the roll angles
		rotatedPos = RotatePointAroundVector( forward, ( toOrg - self.origin ), self.angles[2] * -1 ); // -1 to UNDO the rotation

		// Return pos to its proper position
		toOrg = ( rotatedPos + self.origin );
	}

	vector = toOrg - fromOrg;
	vector = vectornormalize( vector );
	pitchDelta = 360 - vectortoangles( vector )[ 0 ];
