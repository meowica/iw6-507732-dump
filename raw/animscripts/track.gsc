...
trackLoop( aim2, aim4, aim6, aim8 )
{
	...
	else
	{
		...
		if ( ( myScript == "cover_left" || myScript == "cover_right" ) && isdefined( self.a.cornerMode ) && self.a.cornerMode == "lean" )
	...
	for ( ;; )
	{
		...
		if( self isSpaceAI())
		{
			// Fix aim vector for actors who are rotated or rolled on their Z axis.
			roll = self.angles[2] * -1;
			xPitchDelta = (( pitchDelta * Cos(roll)) - ( yawDelta * Sin(roll)));
			xYawDelta = (( pitchDelta * Sin(roll)) + ( yawDelta * Cos(roll)));

			pitchDelta = xPitchDelta;
			yawDelta = xYawDelta;

			// Cap the values in case
...
trackLoop_setAnimWeights( aim2, aim4, aim6, aim8, pitchDelta, yawDelta )
{
	...
	//iprintlnbold( "r:" + (yawDelta / self.rightAimLimit * self.a.aimweight) +
	//			 "l:" + (yawDelta / self.leftAimLimit * self.a.aimweight) +
	//			 "u:" + (pitchDelta / self.upAimLimit * self.a.aimweight) +
	//			 "d:" + (pitchDelta / self.downAimLimit * self.a.aimweight));
}
