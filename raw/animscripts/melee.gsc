...
KNIFE_ATTACK_FX_PATH	=	"fx/impacts/flesh_hit_knife";
...
{
	initAnimSet = [];
	initAnimSet["standard"] = %melee_1;
	initAnimSet["standard_stand_to_melee"] = %stand_2_melee_1;
	initAnimSet["standard_run_to_melee"] = %run_2_melee_charge;

	// Exposed Flip
	initAnimSet["aivai_exposed_attackerwins_attack"] = %melee_F_awin_attack;
	initAnimSet["aivai_exposed_attackerwins_defend"] = %melee_F_awin_defend;
	initAnimSet["aivai_exposed_attackerwins_defend_survive"] = %melee_F_awin_defend_survive;
	initAnimSet["aivai_exposed_defenderwins_attack"] = %melee_F_dwin_attack;
	initAnimSet["aivai_exposed_defenderwins_defend"] = %melee_F_dwin_defend;

	// Exposed Wrestle
	initAnimSet["aivai_wrestle_attackerwins_attack"] = %bog_melee_R_attack;
	initAnimSet["aivai_wrestle_attackerwins_defend"] = %bog_melee_R_defend;
	initAnimSet["aivai_wrestle_attackerwins_defend_survive"] = %bog_melee_R_backdeath2;
	
	// Exposed Behind
	initAnimSet["aivai_behind_attackerwins_attack"] = %melee_sync_attack;
	initAnimSet["aivai_behind_attackerwins_defend"] = %melee_sync_defend;

	// Cover left
	initAnimSet["aivai_coverleft_attackerwins_attack"] = %cornerSdL_melee_winA_attacker;
	initAnimSet["aivai_coverleft_attackerwins_defend"] = %cornerSdL_melee_winA_defender;
	initAnimSet["aivai_coverleft_attackerwins_defend_survive"] = %cornerSdL_melee_winA_defender_survive;
	initAnimSet["aivai_coverleft_defenderwins_attack"] = %cornerSdL_melee_winD_attacker;
	initAnimSet["aivai_coverleft_defenderwins_attack_survive"] = %cornerSdL_melee_winD_attacker_survive;
	initAnimSet["aivai_coverleft_defenderwins_defend"] = %cornerSdL_melee_winD_defender;

	// Cover right
	initAnimSet["aivai_coverright_attackerwins_attack"] = %cornerSdR_melee_winA_attacker;
	initAnimSet["aivai_coverright_attackerwins_defend"] = %cornerSdR_melee_winA_defender;
	initAnimSet["aivai_coverright_defenderwins_attack"] = %cornerSdR_melee_winD_attacker;
	initAnimSet["aivai_coverright_defenderwins_defend"] = %cornerSdR_melee_winD_defender;

	assert( !isdefined( anim.archetypes["soldier"]["melee"] ) );
	anim.archetypes[ "soldier" ]["melee"] = initAnimSet;
}
  ...
  while ( Isdefined( self.melee.target ) )
	{
		...
		meleeAnim = self lookupAnim( "melee", "standard" );
		self setflaggedanimknoballrestart( "meleeanim", meleeAnim, %body, 1, .2, 1 );
		self Melee_PlayFacialAnim( meleeAnim );
...
Melee_Standard_GetInPosition()
{
	...
	enemyDistanceSq = distance_2d_squared( self.origin, self.melee.target.origin );

	if ( enemyDistanceSq <= MELEE_RANGE_SQ )
	{
		// just play a melee-from-standing transition
		meleeAnim = self lookupAnim( "melee", "standard_stand_to_melee" );
		self SetFlaggedAnimKnobAll( "readyanim", meleeAnim, %body, 1, .3, 1 );
		self Melee_PlayFacialAnim( meleeAnim );
	...
	run_to_melee_anim = self lookupAnim( "melee", "standard_run_to_melee" );
	raiseGunAnimTravelDist = length( getmovedelta( run_to_melee_anim, 0, 1 ) );
	...
	raiseGunFullDuration = getanimlength( run_to_melee_anim ) * 1000;
	...
	runAnim = self lookupAnim( "run", "straight" );
	...
	self Melee_PlayFacialAnim( runAnim );
	raisingGun = false;
	...
	while ( 1 )
	{
		...
		if ( !raisingGun )
		{
			...
			{
				Melee_StartMovement();
				self SetFlaggedAnimKnobAllRestart( "chargeanim", run_to_melee_anim, %body, 1, .2, 1 );
				self Melee_PlayFacialAnim( run_to_melee_anim );
		...
		predictedEnemyDistSqAfterRaiseGun = distance_2d_squared( self.origin, predictedEnemyPosAfterRaiseGun );
...
Melee_AIvsAI_Exposed_ChooseAnimationAndPosition_Flip( angleDiff )
{
	...
	{
		self.melee.animName = self lookupAnim( "melee", "aivai_exposed_attackerwins_attack" );
		target.melee.animName = self lookupAnim( "melee", "aivai_exposed_attackerwins_defend" );
		target.melee.surviveAnimName = self lookupAnim( "melee", "aivai_exposed_attackerwins_defend_survive" );
	}
	else
	{
		self.melee.animName = self lookupAnim( "melee", "aivai_exposed_defenderwins_attack" );
		target.melee.animName = self lookupAnim( "melee", "aivai_exposed_defenderwins_defend" );
	}
...
Melee_AIvsAI_Exposed_ChooseAnimationAndPosition_Wrestle( angleDiff )
{
	...
	self.melee.animName = self lookupAnim( "melee", "aivai_wrestle_attackerwins_attack" );
	target.melee.animName = self lookupAnim( "melee", "aivai_wrestle_attackerwins_defend" );
	target.melee.surviveAnimName = self lookupAnim( "melee", "aivai_wrestle_attackerwins_defend_survive" );
...
Melee_AIvsAI_Exposed_ChooseAnimationAndPosition_Behind( angleDiff )
{
	...
	self.melee.winner = true;
	self.melee.animName = self lookupAnim( "melee", "aivai_behind_attackerwins_attack" );
	target.melee.animName = self lookupAnim( "melee", "aivai_behind_attackerwins_defend" );
...
Melee_AIvsAI_SpecialCover_ChooseAnimationAndPosition()
{
	...
	coverNodeType = target.covernode.type;
	if ( coverNodeType == "Cover Multi" )
		coverNodeType = self GetCoverMultiPretendType( target.covernode );

	if ( coverNodeType == "Cover Left" )
	{
		if ( self.melee.winner )
		{
			self.melee.animName = self lookupAnim( "melee", "aivai_coverleft_attackerwins_attack" );
			target.melee.animName = self lookupAnim( "melee", "aivai_coverleft_attackerwins_defend" );
			target.melee.surviveAnimName = self lookupAnim( "melee", "aivai_coverleft_attackerwins_defend_survive" );
		}
		else
		{
			self.melee.animName = self lookupAnim( "melee", "aivai_coverleft_defenderwins_attack" );
			self.melee.surviveAnimName = self lookupAnim( "melee", "aivai_coverleft_defenderwins_attack_survive" );
			target.melee.animName = self lookupAnim( "melee", "aivai_coverleft_defenderwins_defend" );				
		}
	}
	else	// Right
	{
		assert( coverNodeType == "Cover Right" );
		if ( self.melee.winner )
		{
			self.melee.animName = self lookupAnim( "melee", "aivai_coverright_attackerwins_attack" );
			target.melee.animName = self lookupAnim( "melee", "aivai_coverright_attackerwins_defend" );
		}
		else
		{
			self.melee.animName = self lookupAnim( "melee", "aivai_coverright_defenderwins_attack" );
			target.melee.animName = self lookupAnim( "melee", "aivai_coverright_defenderwins_defend" );
		}
	}
...
Melee_AIvsAI_SpecialCover_CanExecute()
{
	...
	if ( coverType == "Cover Multi" )
		coverType = self GetCoverMultiPretendType( cover );
...
Melee_AIvsAI_Execute()
{
	...
	self Melee_PlayFacialAnim( self.melee.animName );
	...
	if ( (endNote == NOTETRACK_DEATH) && isDefined( self.melee.survive ) )
	{
		...
		self Melee_PlayFacialAnim( self.melee.surviveAnimName );
		endNote = self animscripts\shared::DoNoteTracks( "meleeAnim", ::Melee_HandleNoteTracks );
	}
...
Melee_PlayFacialAnim( a_Anim )
{
	self.facialIdx = self animscripts\face::PlayFacialAnim( a_Anim, "pain", self.facialIdx );
}
