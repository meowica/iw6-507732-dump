...
init_animset_reactions()
{
	// Cover reactions
	animset = [];
	animset["cover_stand"] = [ %stand_cover_reaction_A, %stand_cover_reaction_B ];
	animset["cover_crouch"] = [ %stand_cover_reaction_A, %stand_cover_reaction_B ];
	animset["cover_left"] = [ %stand_cover_reaction_A, %stand_cover_reaction_B ];
	animset["cover_right"] = [ %stand_cover_reaction_A, %stand_cover_reaction_B ];
	anim.archetypes[ "soldier" ]["cover_reactions"] = animset;

	// Running bullet reactions
	animset = [];
	animset[0] = %run_react_duck;
	animset[1] = %run_react_flinch;
	animset[2] = %run_react_stumble;
	anim.archetypes[ "soldier" ]["running_react_to_bullets"] = animset;
...
getNewEnemyReactionAnim()
{
	...
	{
		anim_array = self lookupAnimArray( "cover_reactions" );
		if ( isDefined( anim_array[ self.prevScript ] ) )
		{
			...
			{
				self orientmode( "face current" );
				index = randomint( anim_array[ self.prevScript ].size );
				reactAnim = anim_array[ self.prevScript ][ index ];
			}
		}
	}
