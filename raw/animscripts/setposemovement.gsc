...
CrouchStop_Begin()
{
	...
	case "stand":
		...
		case "run":
			...
			break;
		default:
			assertEX( 0, "SetPoseMovement::CrouchStop_Begin " + self.a.pose + " " + self.a.movement );
		}
		break;
	case "crouch":
		switch( self.a.movement )
...
ProneStop_Begin()
{
	...
	case "stand":
		...
		case "run":
			...
			break;
		default:
			assertEX( 0, "SetPoseMovement::CrouchRun_Begin " + self.a.pose + " " + self.a.movement );
		}
		break;
	case "crouch":
		switch( self.a.movement )
...
BlendIntoStandRun()
{
	...
	if ( isdefined( self.sprint ) )
		self setAnimKnobLimited( GetMoveAnim( "sprint" ), 1, runAnimTransTime, 1 );
	else
		self setAnimKnobLimited( animscripts\run::GetRunAnim(), 1, runAnimTransTime, 1 );

	self animscripts\run::SetMoveNonForwardAnims( GetMoveAnim( "move_b" ), GetMoveAnim( "move_l" ), GetMoveAnim( "move_r" ), self.sideStepRate );
...
BlendIntoCrouchRun()
{
	...
	else
	{
		...
		self thread animscripts\run::UpdateMoveAnimWeights( "crouchrun", GetMoveAnim( "crouch" ), GetMoveAnim( "crouch_b" ), GetMoveAnim( "crouch_l" ), GetMoveAnim( "crouch_r" ) );
