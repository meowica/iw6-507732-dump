...
initAnimTree( animscript )
{
	...
	//self.missedSightChecks = 0;       // <-- don't see any references anywhere...
...
UpdateAnimPose()
{
	...
	if ( isDefined( self.desired_anim_pose ) && self.desired_anim_pose != self.a.pose )
	{
		...
		if ( self.desired_anim_pose == "prone" )
		{
			...
			self setAnimKnobAll( self lookupAnim( "default_prone", "straight_level" ), %body, 1, 0.1, 1 );
		}
	}
...
initialize( animscript )
{
	...
	{
		self animscripts\combat::switchToLastWeapon( self lookupAnim( "combat", "pistol_to_primary" ), true );
...
// any second.
IsInCombat( bCheckAlertLevel )
{
	if ( ( !IsDefined( bCheckAlertLevel ) || bCheckAlertLevel ) && self.alertLevelInt > 1 )
		return true;
...
GetYawToSpot( spot )
{
	// Space must account for actors with roll angles
	if( self isSpaceAI())
	{
		forward = AnglesToForward( self.angles );

		// Move pos to origin, and rotate it around self by the roll angles
		rotatedPos = RotatePointAroundVector( forward, ( spot - self.origin ), self.angles[2] * -1 );

		// Return pos to its proper position
...
GetYawToOrigin( org )
{
	// Be absolutely sure this a SPACE cover node
	if( isDefined( self.type )
	   && ( self.type == "Cover Up 3D" || self.type == "Cover Left 3D" || self.type == "Cover Right 3D" || self.type == "Exposed 3D" ) 
	   && isDefined( self.script_angles ) && self.script_angles[2] != 0 )
	{
		// Space must account for nodes with roll angles
		forward = AnglesToForward( self.script_angles );

		// Move org to origin, and rotate it around self by the roll angles
		rotatedOrg = RotatePointAroundVector( forward, ( org - self.origin ), self.script_angles[2] * -1 );

		// Return org to its proper position
		rotatedOrg = ( rotatedOrg + self.origin );

		// Find the yaw difference
		yaw = self.script_angles[ 1 ] - GetYaw( rotatedOrg );
		yaw = AngleClamp180( yaw );
		return yaw;
	}
...
shootEnemyWrapper( bFireSoundOnce )
{
	if ( !IsDefined( bFireSoundOnce ) )
		bFireSoundOnce = true;
	[[ anim.shootEnemyWrapper_func ]]( bFireSoundOnce );
}

shootEnemyWrapper_normal( bFireSoundOnce )
{
	...
	self shoot( 1, undefined, bFireSoundOnce );
	prof_end( "code shoot" );
}

shootEnemyWrapper_shootNotify( bFireSoundOnce )
{
	level notify( "an_enemy_shot", self );
	shootEnemyWrapper_normal( bFireSoundOnce );
}

shootPosWrapper( shootPos, bFireSoundOnce )
{
...
isSpaceAI()
{
	return(( isDefined( self.swimmer ) && self.swimmer == true ) && ( isDefined( self.space ) && self.space == true ));
}
...
getNodeOffset( node )
{
	...
	if ( nodeType == "Cover Multi" )
		nodeType = self GetCoverMultiPretendType( node );
...
{
	if ( IsNodeCoverLeft( node ) || IsNodeCoverRight( node ) || IsNodeCoverMultiCorner( node ) )
	{
	...
	if ( !sightTracePassed( lookFromPoint, point, false, undefined ) )
	{
		...
		// Space AI cover_up behaves similar to crouch, with a lower offset
		if( self isSpaceAI() && node.type == "Cover Up 3D" )
		{
			actor_up = AnglesToUp( self.angles );
			lookFromPoint = node.origin + ( actor_up[0] * 32, actor_up[1] * 32, actor_up[2] * 32 );
			return sightTracePassed( lookFromPoint, point, false, undefined );
		}
...
IsNodeCoverMultiCorner( node )
{
	if ( node.type != "Cover Multi" )
		return false;
...
GetCoverMultiPretendType( node, state )
{
	assert( node.type == "Cover Multi" );

	if ( !IsDefined( state ) )
	{
		state = node animscripts\cover_multi::CoverMulti_GetNonRandomValidDir();
...
setFootstepEffect( name, fx )
{
	...
	anim.optionalStepEffects[ name ] = true;
...
setFootstepEffectSmall( name, fx )
{
	...
	anim.optionalStepEffectsSmall[ name ] = true;
...
//	}
	...
	return IsDefined( self.a.array[ animname ] ) && self.a.array[ animname ].size > 0;
}
...
shouldDropRocketLauncher( drop_distSqrd )
{
	AssertEx( self usingRocketLauncher(), "shouldDropRocketLauncher() called on ai not currently holding rocket launcher." );
	
	return IsDefined( self.secondaryweapon ) && self.secondaryweapon != "none" && ( drop_distSqrd < squared( 512 ) || self.a.rockets < 1 );
}
...
IsNodeCoverLeft( node )
{
    ...
    return node.type == "Cover Left" || node.type == "Cover Left 3D";
}

IsNodeCoverRight( node )
{
    assert( IsDefined( node ) && IsDefined( node.type ) );
    return node.type == "Cover Right" || node.type == "Cover Right 3D";
}

IsNodeCover3D( node )
{
	assert( IsDefined( node ) && IsDefined( node.type ) );
	return node.type == "Cover Left 3D" || node.type == "Cover Right 3D" || node.type == "Cover Up 3D";
}
...
lookupAnim( animset_name, anim_index )
{
	...
	AssertEx( IsDefined( anim.archetypes["soldier"][animset_name][anim_index] ), "Missing anim for soldier archetype: [" + animset_name + "][" + anim_index + "]" );
...
lookupAnimArray( animset_name )
{
	...
	if ( IsDefined( self.animArchetype ) )
	{
		...
		{
			// Combine with the default animset so every key is represented
	...
	AssertEx( IsDefined( anim.archetypes["soldier"][animset_name] ), "Missing anim array for soldier archetype: [" + animset_name + "]" );
	return anim.archetypes["soldier"][animset_name];
}

lookupTransitionAnim( animset_name, anim_index, direction_index )
{
	assert( IsAI( self ) );
	if ( IsDefined( self.animArchetype ) )
	{
		assertEx( IsDefined( anim.archetypes[self.animArchetype] ), "Invalid anim archetype: " + self.animArchetype );
		if ( IsDefined( anim.archetypes[self.animArchetype][animset_name] ) && IsDefined( anim.archetypes[self.animArchetype][animset_name][anim_index] ) && IsDefined( anim.archetypes[self.animArchetype][animset_name][anim_index][direction_index] ) )
		{
			return anim.archetypes[self.animArchetype][animset_name][anim_index][direction_index];
		}
	}

	AssertEx( IsDefined( anim.archetypes["soldier"][animset_name][anim_index][direction_index] ), "Missing transition anim for soldier archetype: [" + animset_name + "][" + anim_index + "]" + "[" + direction_index + "]" );
	return anim.archetypes["soldier"][animset_name][anim_index][direction_index];
}

lookupDogAnim( animset_name, anim_index )
{
	assert( IsAI( self ) );
	if ( IsDefined( self.animArchetype ) )
	{
	...
	AssertEx( IsDefined( anim.archetypes["dog"][animset_name][anim_index] ), "Missing anim for dog archetype: [" + animset_name + "][" + anim_index + "]" );
	return anim.archetypes["dog"][animset_name][anim_index];
}
