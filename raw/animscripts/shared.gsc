...
moveToNodeOverTime( node, time )
{
	...
	// use nodeOrigin here to save on repeated calculations of node.origin.
	...
	if ( distSq > 16 * 16 && !self mayMoveToPoint( nodeOrigin, !self.swimmer ) )
	{
		 /# println( "^1Warning: AI starting behavior for node at " + nodeOrigin + " but could not move to that point." ); #/
		return;
	}
	...
	// lerp from self.origin to node.origin
	for ( i = 0; i < frames; i++ )
	{
		nodeOrigin = node.origin;
		...
		//self safeTeleport( node.origin + offset );	// use node.origin here because it can change between frames.
		wait .05;
	}
