...
init_battleChatter()
{
	...
	// RPG/exposed
	anim.threatCallouts[ "rpg" ] 						 = 80;// "RPG!"
	...
	*/
	...
	anim.threatCallouts[ "player_location" ]			 = 90;// ( Player - relative ) "Contact! 2nd floor window on the left!"
...
bcs_setup_countryIDs()
{
	if ( !IsDefined( anim.usedIDs ) )
	{
		...
		bcs_setup_voice( "american", "US", 3, true );
//		bcs_setup_voice( "seal", "NS", 4 );
		bcs_setup_voice( "taskforce", "TF", 1 );
		bcs_setup_voice( "secretservice", "SS", 4 );
		bcs_setup_voice( "delta", "DF", 3 );
		bcs_setup_voice( "french", "FR", 3 );
		bcs_setup_voice( "czech", "CZ", 3 );
		bcs_setup_voice( "pmc", "PC", 3 );
		
		bcs_setup_voice( "russian", "RU", 5 );
		bcs_setup_voice( "arab", "AB", 3 );
		bcs_setup_voice( "portugese", "PG", 3 );
//		bcs_setup_voice( "shadowcompany", "SC", 4, true );
		bcs_setup_voice( "african", "AF", 3 );

		// NEW VOICES
		bcs_setup_voice( "seal", "GS", 1 );	
		
		bcs_setup_voice( "shadowcompany", "SP", 1 );
	}
}

bcs_setup_voice( voice, countryID, numVoices, allowFlavorburst )
{
	if ( !isdefined( allowFlavorburst ) )
		allowFlavorburst = false;
	...
	for ( i=0; i<numVoices; i++ )
	{
		...
		anim.usedIDs[ voice ][ i ].npcID = "" + i;
	}
	
	anim.countryIDs[ voice ]		= countryID;
	anim.flavorburstVoices[ voice ] = allowFlavorburst;
}
...
//		break;
	...
	*/
	...
	case "concat_location":
		chatPhrase addConcatTargetAlias( threat );
		...
		success = chatPhrase threatInfantry_doCalloutLocation( callout, true );
		if ( !success )
		{
			return false;
		}
		
		chatPhrase addConcatDirectionAlias( relativeGuy, threat );
		
		// "yes" or "no" for now, should be location echo eventually
		chatPhrase addCalloutResponseEvent( self, callout, threat );
		break;
...
threatInfantry_doCalloutLocation( callout, concat )
{
	success = self addThreatCalloutLocationAlias( callout.location, concat );
...
{
	//prof_begin( "getThreatInfCallout" );
	// get info about the AI
	location = threat GetValidLocation( self );
...
//	threatInFrontArc = level.player entInFrontArc( threat );	
	...
	if ( !IsDefined( location ) && threat IsExposed( false ) )
	{
	...
	*/
	...
	{
		if ( self canConcat( location ) )
		{
			self addPossibleThreatCallout( "concat_location" );
		}
		else
		{
			if ( IsDefined( location GetCannedResponse( self ) ) )
			{
...
{
	alias = self.countryID + "_" + self.npcID + "_";
	alias += basealias;
...
{
	alias = self GetLocCalloutAlias( "co_loc_" + basealias );
	alias += "_qa" + lineIndex;
...
threatInfantryExposed( threat )
{
	...
	if ( self.owner.countryID != "GS" )
		exposedVariants = array_add( exposedVariants, "breaking" ); //no "breaking from cover" callouts if you're in stealth
...
isExposed( bCheckLocation )
{
	...
	if ( IsDefined( bCheckLocation ) && bCheckLocation && IsDefined( self GetLocation() ) )
	{
		return false;
	}
...
addConcatTargetAlias( threat )
{
	targetType = "";
	
	if ( threat usingRocketLauncher() )
		targetType = "_rpg";
	else if ( threat bcIsSniper() )
		targetType = "_sniper";
	
	nameAlias = self.owner.countryID + "_" + self.owner.npcID + "_concat_target" + targetType;

	self.soundAliases[ self.soundAliases.size ] = nameAlias;
}

addConcatDirectionAlias( relativeGuy, threat )
{
	directionType = random( [ "relative", "absolute" ] );
	switch ( directionType )
	{
		case "absolute":
			cardinalDirection = getDirectionCompass( level.player.origin, threat.origin );
			normalizedDirection = normalizeCompassDirection( cardinalDirection );
			if ( normalizedDirection != "impossible" && normalizedDirection.size != 2 )
			{
				alias = self.owner.countryID + "_" + self.owner.npcID + "_concat_dir_compass_" + normalizedDirection;
		...
		case "relative":
			angles = getRelativeAngles( relativeGuy );
			clockDirection = getDirectionFacingClock( angles, relativeGuy.origin, threat.origin );
			clockDirectionInt = Int( clockDirection );
			if ( 1 < clockDirectionInt && clockDirectionInt < 5 && cointoss() )
			{
				alias = self.owner.countryID + "_" + self.owner.npcID + "_concat_dir_right";
				self.soundAliases[ self.soundAliases.size ] = alias;
				break;
			}
			else if ( 7 < clockDirectionInt && clockDirectionInt < 11 && cointoss() )
			{
				alias = self.owner.countryID + "_" + self.owner.npcID + "_concat_dir_left";
				self.soundAliases[ self.soundAliases.size ] = alias;
				break;
			}
			else
			{
				alias = self.owner.countryID + "_" + self.owner.npcID + "_concat_dir_clock_" + clockDirection;
				self.soundAliases[ self.soundAliases.size ] = alias;
			}
			break;
	}
...
addThreatCalloutQA_NextLine( respondTo, prevLine, location )
{
	...
	qaAlias = prefix + "co_loc_" + partialAlias + "_qa" + nextIndex;
...
// format: US_1_[location.locationAliases[idx]]
...
addThreatCalloutLocationAlias( location, concat )
{
	...
	if ( isdefined( concat ) && concat )
	{
		alias = self.owner GetLocCalloutAlias( "concat_loc_" + locAlias );
	}
	else if ( !IsDefined( location.qaFinished ) && IsCalloutTypeQA( locAlias, self.owner ) )
	{
		alias = self.owner GetQACalloutAlias( locAlias, 0 );
	}
	else
	{
		alias = self.owner GetLocCalloutAlias( "co_loc_" + locAlias );
	}
