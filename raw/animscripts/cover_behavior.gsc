...
{
//	if ( IsDefined( self.cover ) && IsDefined( self.cover.iStateChange ) )
//		self endon( "covermulti_setstate_" + self.cover.iStateChange );
	...
	for ( ;; )
	{
		...
		// Do not teleport with angles on space characters
		if( self isSpaceAI())
		{
...
set_standing_turns()
{
	anim_array = self lookupAnimArray( "exposed_turn" );
	foreach ( key, value in anim_array )
	{
...
set_crouching_turns()
{
	anim_array = self lookupAnimArray( "exposed_turn_crouch" );
	foreach ( key, value in anim_array )
	{
		self.a.array[ key ] = value;
	}
