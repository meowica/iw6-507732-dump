...
init_aiBattleChatter()
{
	...
	if( self.team == "allies" )
	{
		...
		if( self.voice == "delta" || self.voice == "taskforce" )
		{
			self addAllowedThreatCallout( "concat_location" );
...
GetThreatsOverTime( potentialThreats, time )
{
	nThreats = potentialThreats.size;
	if ( nThreats == 0 )
	{
		wait( time );
	...
	nFrames = time * 20;
	nThreatsPerFrame = nThreats / nFrames;

	threats = [];

	for ( i = 0; i < nThreats; i++ )
	{
		potentialThreat = potentialThreats[ i ];
		if ( !IsDefined( potentialThreat.enemyClass ) )
			continue;
	...
	{
		if ( IsDefined( threat ) && ThreatIsViable( threat ) )
		{
			location = threat GetLocation();       // <-- expensive call we are trying to spread out over the frames.
		...
		nThreatsProcessed++;
		if ( nThreatsProcessed >= nThreatsPerFrame )
		{
			wait( 0.05 );
			nThreatsProcessed = 0;
		}
...
squadThreatWaiter()
{
	...
	while ( 1 )
	{
		...
		if ( self.team == "allies" )
		{
			validEnemies = GetThreatsOverTime( GetAIArray( "axis", "team3" ), 0.5 );
		}
		else if ( self.team == "team3" )
		{
			validEnemies = GetThreatsOverTime( GetAIArray( "allies", "axis" ), 0.5 );
		}
		else
		{
            wait( 0.5 );
...
aiFollowOrderWaiter()
{
	...
	while ( true )
	{
		...
		if ( !isAlive( speaker ) || speaker.team != self.team )
			continue;
