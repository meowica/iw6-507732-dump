...
MoveCQB()
{
	...
	self CQB_PlayFacialAnim( cqbWalkAnim );

	self animscripts\run::SetMoveNonForwardAnims( self lookupAnim( "cqb", "move_b" ), self lookupAnim( "cqb", "move_l" ), self lookupAnim( "cqb", "move_r" ) );
...
DetermineCQBAnim()
{
	...
	if ( self.stairsState == "up" )
		return self lookupAnim( "cqb", "stairs_up" );

	if ( self.stairsState == "down" )
		return self lookupAnim( "cqb", "stairs_down" );

	if ( self.movemode == "walk" )
		return self lookupAnim( "cqb", "move_f" );

	variation = getRandomIntFromSeed( self.a.runLoopCount, 2 );
	if ( variation == 0 )
		return self lookupAnim( "cqb", "straight" );

	return self lookupAnim( "cqb", "straight_v2" );
}
...
CQBTracking()
{
	...
	bMayShoot = animscripts\move::MayShootWhileMoving();
	animscripts\run::SetShootWhileMoving( bMayShoot );

	self thread animscripts\run::FaceEnemyAimTracking();
}
