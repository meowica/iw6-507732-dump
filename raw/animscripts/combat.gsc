...
{
	initAnimSet = [];
	initAnimSet["surprise_stop"] = %surprise_stop_v1;
	initAnimSet["trans_to_combat"] = %casual_stand_idle_trans_out;
	initAnimSet["drop_rpg_stand"] = %RPG_stand_throw;
	initAnimSet["drop_rpg_crouch"] = %RPG_crouch_throw;
	initAnimSet["pistol_to_primary"] = %pistol_stand_switch;
	initAnimSet["primary_to_pistol"] = %pistol_stand_pullout;
	initAnimSet["rpg_death"] = %RPG_stand_death;
	initAnimSet["rpg_death_stagger"] = %RPG_stand_death_stagger;

	assert( !isdefined( anim.archetypes["soldier"]["combat"] ) );
	anim.archetypes[ "soldier" ]["combat"] = initAnimSet;
}
...
DoFriendlyFireReaction()
{
	...
	if ( self IsMoveSuppressed() && self.prevScript == "move" && self.a.pose == "stand" && !IsDefined( self.disableFriendlyFireReaction ) )
	{
		...
		{
			surprise_anim = animarray( "surprise_stop" );
		}
		else if ( self.swimmer )
		{
			surprise_anim = self animscripts\swim::GetSwimAnim( "surprise_stop" );
			if ( !IsDefined( surprise_anim ) )
				return;
		}
		else
		{
			surprise_anim = self lookupAnim( "combat", "surprise_stop" );
		}
		
		self ResetAnimMode();
		self setFlaggedAnimKnobAllRestart( "react", surprise_anim, %root, 1, 0.2, self.animplaybackrate );
		self Combat_PlayFacialAnim( surprise_anim, "run" );
		DoNoteTracks( "react" );
	}
}
...
TransitionToCombat()
{
	...
	if ( self.prevScript == "stop" && !self IsCQBWalking() && self.a.pose == "stand" )
	{
		...
		trans_anim = self lookupAnim( "combat", "trans_to_combat" );
		self SetFlaggedAnimKnobAllRestart( "transition", trans_anim, %root, 1, 0.2, 1.2 * self.animPlaybackRate );
		self Combat_PlayFacialAnim( trans_anim, "run" );
...
set_default_aim_limits( node )
{
	if ( IsDefined( node ) )
		self setDefaultAimLimits( node );
	else
		self setDefaultAimLimits();
	self set_default_swimmer_aim_limits();
}

set_aim_and_turn_limits()
{
	//We have a slightly greater pitch reach when stand_exposed
	self set_default_aim_limits();	
...
exposedCombatStopUsingRPGCheck( distSqToShootPos )
{
	...
	if ( usingRocketLauncher() && shouldDropRocketLauncher( distSqToShootPos ) )
	{
		...
		// make sure this guy doesn't have an RPG in each hand.  that makes it hard to stop using the RPG.
		Assert( IsDefined( self.secondaryweapon ) && WeaponClass( self.secondaryweapon ) != "rocketlauncher" );
...
exposedCombatCheckPutAwayPistol( distSqToShootPos )
{
	if ( usingSidearm() && self.a.pose == "stand" && !isdefined( self.forceSideArm ) )
		if ( ( distSqToShootPos > pistolPutBackDistSq ) || ( self.combatMode == "ambush_nodes_only" && ( !isdefined( self.enemy ) || !self cansee( self.enemy ) ) ) )
			switchToLastWeapon( self lookupAnim( "combat", "pistol_to_primary" ) );
}
...
{
	self endon( "killanimscript" );
	//self endon( "combat_restart" );	// <-- i don't see anything that sends out this notification.

	self setupExposedCombatLoop();

	self ResetAnimMode( false );
	...
	for ( ;; )
	{
		...
		self UpdateIsInCombatTimer();// reset our in - combat state
		...
		if ( !IsDefined( self.shootPos ) )
		{
			...
			//	justWaited = true;			// <- doesn't seem to be used anywhere.
			continue;
		}
...
WaitForStanceChange()
{
	...
	if ( !self isStanceAllowed( curstance ) )
	{
		...
		else if ( self isStanceAllowed( laststance ) )
		{
			transitionTo( laststance );
			return true;
		}
		else
		{
			assertex( false, "Actor has no allowable stances!");
		}
	}
	return false;
}
...
cantSeeEnemyBehavior()
{
	...
	if ( givenUpOnEnemy && usingSidearm() )
	{
		...
		if( switchToLastWeapon( self lookupAnim( "combat", "pistol_to_primary" ) ) )
			return true;
	}
...
doTurn( direction, amount )
{
	...
	if ( isdefined( self.turnToMatchNode ) )
		...
		self ResetAnimMode();	//animMode( "zonly_physics", false );
	else
		self animmode( "angle deltas", false );
	...
	self Combat_PlayFacialAnim( turnAnim, "aim" );
	self notify( "turning" );
	...
	if ( distanceSquared( self.origin, throwSpot ) > minDist * minDist && self.a.pose == self.a.grenadeThrowPose )
	{
		...
		if ( abs( yaw ) < 60 )
		{
			...
			if ( throwAnims.size > 0 )
			{
				...
				self Combat_PlayFacialAnim( undefined, "aim" );
...
transitionTo( newPose )
{
	...
	self Combat_PlayFacialAnim( transAnim, "run" );
...
doReloadAnim( reloadAnim, stopWhenCanShoot )
{
	...
	self Combat_PlayFacialAnim( reloadAnim, "run" );
...
tryUsingSidearm()
{
	...
	switchToSidearm( self lookupAnim( "combat", "primary_to_pistol" ) );
...
switchToSidearm( swapAnim )
{
	...
	self Combat_PlayFacialAnim( swapAnim, "run" );
...
rpgDeath()
{
	...
	if ( randomFloat( 1 ) > 0.5 )
		deathAnim = self lookupAnim( "combat", "rpg_death" );
	else
		deathAnim = self lookupAnim( "combat", "rpg_death_stagger" );

	self SetFlaggedAnimKnobAll( "deathanim", deathAnim, %root, 1, .05, 1 );

	self Combat_PlayFacialAnim( deathAnim, "death" );
...
ResetAnimMode( a_bResetPath )
{
	bResetPath = a_bResetPath;
	if ( !IsDefined( bResetPath ) )
		bResetPath = true;

	if ( self.swimmer )
		self AnimMode( "nogravity", bResetPath );
	else
		self AnimMode( "zonly_physics", bResetPath );
}

Combat_PlayFacialAnim( a_Anim, a_State )
{
