...
/#
	...
	foreach ( ent in ents )
	{
		if ( IsSubStr( ent.classname, "actor_template_" ) )
			AssertMsg( "spawner or AI with classname of \"" + ent.classname + "\" at " + ent.origin + " is not supported. Please use a proper AItype" );
	}
#/
}
...
trigger_pool_spawners( target )
{
	...
	//assume this trigger is only triggered once.
...
get_spawner_from_pool( struct_spawner, max_pool_size )
{
	...
	{
		// face them in the direction of the node ( save some variables on the struct )
...
dronespawner_init()
{
	self maps\_drone_base::drone_init_path();
}
...
=============
///ScriptFieldDocBegin
"Name: .finished_spawning"
"Summary: finished_spawning"
"Module: spawner"
"SPMP: singleplayer"
///ScriptDocEnd
=============
...
multikill_monitor()
{
	...
	// quad kill survival challenge notify
...
deathFunctions()
{
	...
	if ( IsDefined( attacker ) )
	{
		if ( self.team == "axis" || self.team == "team3" )
		{
			...
			// Pass attacker.attacker (this can be from anything... wild west scripting!)
			if ( isdefined( attacker.attacker ) )
			{
				// If the attacker is a sentry gun
...
spawn_think_script_inits()
{
	...
	{
		assertEx( self.script_ignoreme == true, "script_ignoreme set to something other than true (1) in Radiant. The only valid values are true or undefined. Defaulting to true." );
		self.ignoreme = true;
	}

	if ( isdefined( self.script_ignore_suppression ) )
	{
		assertEx( self.script_ignore_suppression == true, "script_ignore_suppression set to something other than true (1) in Radiant. The only valid values are true or undefined. Defaulting to true." );
		self.ignoreSuppression = true;
	}

	if ( isdefined( self.script_ignoreall ) )
	{
		assertEx( self.script_ignoreall == true, "script_ignoreall set to something other than true (1) in Radiant. The only valid values are true or undefined. Defaulting to true." );
		self.ignoreall = true;
		self clearenemy();
	}
...
spawn_think_action( targetname )
{
	...
	if ( !isdefined( self.script_stealth ) ) // this is because the stealth system could possibly have changed the goalradius.  gross, so hacky.  -Carlos
...
// friendly chains have been cleared out from code.
// this and all its little buddy functions can probably be deleted as well.
friendlyChains()
{
...
flood_spawner_stop()
{
	AssertEx( IsDefined( self ) && IsSpawner( self ), "flood_spawner_stop() called with self that is not a spawner." );
	self notify( "stop current floodspawner" );
}
...
//	thread achieve_ten_plus_hellfire( attacker );->moved to _REMOVEMISSLE
	...
	// anything that should happen with long death goes after here
}
...
{	
	...
	// only thrown grenades, not grenade launchers
...
{
	// Finish a Juggernaut with a knife
	if (
...
{
	// not used in Modern 3, but some helicopter code calls us directly.
}
