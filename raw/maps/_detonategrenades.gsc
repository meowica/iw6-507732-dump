...
semtex_sticky_handle( attacker )
{
	...
	if( 
		...
		|| 	entity maps\_vehicle_code::attacker_isonmyteam( attacker ) 
		)
	{
...
begin_concussion_grenade_tracking()
{
	...
	{
		self waittill( "grenade_fire", grenade, weaponName );
		if ( weaponName == "concussion_grenade" )
		{
			thread track_concussion_grenade( grenade );
		}
	}
}
	
track_concussion_grenade( grenade )
{
	self.throwingGrenade = false;
...
beginGrenadeTracking()
{
	...
	else if ( weaponName == "ninebang_grenade" )	// We want to adjust the ninebang radius but keep it the same for the player
