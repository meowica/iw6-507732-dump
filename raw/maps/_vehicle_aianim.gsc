...
guy_enter( guy, climbed_in_vehicle )
{
    ...
    link_to_sittag( guy, animpos.sittag, animpos.sittag_offset, animpos.linktoblend );
...
{
    if ( is_rider( guy ) )
        return; // this guy is already riding!
...
guy_runtovehicle( guy, vehicle, bGoddriver, group )
{
    ...
    vehicle link_to_sittag( guy, animpos.sittag, animpos.sittag_offset, animpos.linktoblend );
...
//      }
        ...
        if ( IsDefined( animpos.getin_enteredvehicletrack ) )
        {
            ...
            vehicle link_to_sittag( guy, animpos.sittag, animpos.sittag_offset, animpos.linktoblend ); // link them, normally they don't link for whatever reason I don't want tof ind otu.
        }
...
link_to_sittag( guy, tag, offset, linktoblend )
{
	if ( !IsDefined( offset ) )
		offset = ( 0, 0, 0 );
...
guy_turret_fire( guy, pos )
{
    ...
    if ( IsDefined( animpos.vehicle_turret_fire ) )
    	self _get_dummy() SetAnimRestart( animpos.vehicle_turret_fire );
...
getoutrig_disable_abort_notify_after_riders_out()
{
    ...
    while ( IsAlive( self ) && self.unloadque.size > 2 )
        wait 0.05;// 1 unloadque will be there for the rope.
...
getoutrig_abort( model, tag, animation )
{
    ...
    if ( self.vehicletype == "mi17" )
        ropesFallAnimTime = totalAnimTime - 0.5;// go go ghetto numbers
...
guy_unload( guy, pos )
{
    ...
    if ( !bNoanimUnload )
    {
    	//dont unlink parachute guys from their chutes 
    	if ( !IsDefined( guy.parachute_unload ) )
    	{
        ...
        //this is for the parachute unloaders
        else if ( IsDefined( animpos.parachute_unload ) )
		{
        	...
        	if ( !IsDefined( self ) ) //check after waiting
        		return;
			new_tag		   = Spawn( "script_model", self GetTagOrigin( tag ) );
			new_tag.angles = ( 0, self.angles[ 1 ] + self.angle_offset, 0 ); //must be flat angles of vehicle to align anim correctly!
        	self.angle_offset += 5;
        	new_tag SetModel( "tag_origin" );
        	
			required_height = groundpos( self.origin ) + ( 0, 0, self.unload_anim_height );
			new_tag.origin	= required_height + ( RandomIntRange( 10, 20 ), RandomIntRange( 10, 20 ), 0 );
        	
			parachute		 = Spawn( "script_model", new_tag.origin );
			parachute.angles = new_tag.angles;
			parachute SetModel( self.unload_model	 ); //set in the vehicles script	
			parachute.animname = "parachute";
			parachute UseAnimTree( level.scr_animtree[ "parachute" ] );	//set in the vehicle script	
			parachute Hide();
			parachute ent_flag_init( "parachute_open" );
			
			if ( IsDefined( animpos.parachute_function ) )
			{
				new_tag parachute_unload( guy, parachute, self.unload_model_unload_anim, animation, animpos.parachute_function );
			}
			else
			{
       			new_tag parachute_unload( guy, parachute, self.unload_model_unload_anim, animation );
			}
...
parachute_unload( guy, parachute_model, parachute_anim, guy_anim, optional_notetrack_func )
{
	//-Parachute animates off spawned tag_origin (self) 
	//-guys anim is relative and he is linked to the chute
	
	guy Unlink();
	origin = parachute_model GetTagOrigin( "tag_driver" );
	angles = parachute_model GetTagAngles( "tag_driver" );
	
	guy ForceTeleport( origin, angles );	
	guy LinkTo( parachute_model, "tag_driver" );
	
	//parachute_model thread parachute_movement( guy.ridingvehicle );
	
	parachute_model AnimScripted( "parachute_unload", self.origin, self.angles, parachute_anim );
		
	if ( IsDefined( optional_notetrack_func ) )
	{
		parachute_model thread parachute_notetrack_logic( "parachute_unload", "show_parachute", optional_notetrack_func );
	}
	else
	{
		parachute_model thread parachute_notetrack_logic( "parachute_unload", "show_parachute" );
	}

	guy AnimScripted( "parachute_unload", guy.origin, guy.angles, guy_anim );
	level thread parachute_death_monitor( guy, parachute_model );
	
	parachute_model waittillmatch( "parachute_unload", "end" );
	
	//anim is complete	
	parachute_model notify( "parachute_landed" );
...
parachute_movement( chopper )
{
	//self = spawned ent parachute is animating off
	//wait(.10);
	fwd			   = AnglesToForward( chopper.angles );
	forwardvec	   = VectorNormalize( flat_angle( fwd ) );
	new_forwardvec = self.origin +( forwardvec * 10000 );
	thread draw_line_from_ent_for_time( self, new_forwardvec, 1, 0, 0, 10 );
	self MoveTo( new_forwardvec, 1 );
}

parachute_death_monitor( guy, parachute_model )
{
	//copied and modified from animontag_ragdoll_death()
	parachute_model endon( "parachute_landed" );
    if ( IsDefined( guy.magic_bullet_shield ) && guy.magic_bullet_shield )
        return;
    if ( !IsAI( guy ) )
        guy SetCanDamage( true );
   ...
   if ( !parachute_model ent_flag( "parachute_open" ) )
   {
   		//chute not open yet. 
   		parachute_model notify( "rider_dead" ); //aborts the chute getting show()
   		thread animontag_ragdoll_death_fall( guy, undefined, attacker );		
   		wait( 2 );
   		parachute_model Delete();
   }
   else
   {
...
parachute_notetrack_logic( flag, notetrack, function )
{
  self endon ( "rider_dead" ); //dont show the chute if guy gets killed prior to it opening
  self waittillmatch( flag, notetrack );	
  if ( IsDefined( function ) )
  {
 	 self thread [[ function ]]();	
  }
  self ent_flag_set( "parachute_open" );
}
...
animontag_ragdoll_death_fall( guy, vehicle, attacker )
{
    ...
    if( IsDefined( guy.script_stay_drone ) )
	{
		...
		if ( IsDefined( weapon_model ) )
		{
			...
			level.gun		   = Spawn( "weapon_" + weapon, ( 0, 0, 0 ) );
...
guy_vehicle_death( guy, attacker, type )
{
    ...
    if ( IsDefined( level.vehicle_rider_death_func ) && IsDefined( level.vehicle_rider_death_func[ self.vehicletype ] ) )
	{
		self [[ level.vehicle_rider_death_func[ self.vehicletype ] ]]();
		return;
    }
