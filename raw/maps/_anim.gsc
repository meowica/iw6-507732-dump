...
#/
	...
	self endon( ender );
/#
	self thread looping_anim_ender( baseGuy, ender, anime );
#/
...
// 			guy thread animsound_start_tracker( anime );
			...
			if ( doAnimation )
			{
				if ( ( IsDefined( level.scr_goaltime[ animname ] ) ) &&
					( IsDefined( level.scr_goaltime[ animname ][ anime ] ) ) )
					goalTime = level.scr_goaltime[ animname ][ anime ];
				else
					goalTime = 0.2;

				// ai and models use animscripted
				guy last_anim_time_check();
				guy AnimScripted( anim_string, org, angles, level.scr_anim[ animname ][ anime ][ idleanim ], undefined, undefined, goalTime );
...
anim_single_internal( guys, anime, tag, anim_end_time, animname_override )
{
	...
	foreach ( i, guy in guys )
	{
		...
		/#
		guy assert_existance_of_anim( anime, animname );
		AssertEx( !IsArray( level.scr_anim[ animname ][ anime ] ), "level.scr_anim[ \""+animname+"\" ][ \""+anime+"\" ] is an array but was called with an anim_ function that does not support arrays.  You might need to get rid of the [ 0 ] where you define it in your _anim.gsc file." );
		#/
...
//				guy ForceTeleport( org, angles );
				...
				guy SetAnim( root_animation, 0, goalTime );

				animation = level.scr_anim[ animname ][ anime ];
				guy SetFlaggedAnim( anim_string, animation, 1, goalTime );
			...
			{
				// ai and models use animscripted
				guy AnimScripted( anim_string, org, angles, level.scr_anim[ animname ][ anime ], undefined, undefined, goalTime );
			}
...
// 	self endon( ender );
	...
	while ( 1 )
	{
		...
		if ( prefix == "fx_" )
		{
			str = StrTok( ToLower( GetSubStr( notetrack, 3 ) ), ",() " );
			...
			if ( str.size == 2 )
			{
				...
				{
					PlayFXOnTag( level._effect[ str[ 0 ] ], guy, str[ 1 ] );
					continue;
				}
			}
			else if ( str.size == 3 )
			{
				...
				{
					PlayFXOnTag( level._effect[ str[ 1 ] ], guy, str[ 2 ] );
					continue;
				}
				else if ( str[ 0 ] == "stopfxontag" )
				{
					StopFXOnTag( level._effect[ str[ 1 ] ], guy, str[ 2 ] );
					continue;
				}
				else if ( str[ 0 ] == "killfxontag" )
				{
					KillFXOnTag( level._effect[ str[ 1 ] ], guy, str[ 2 ] );
					continue;
				}
			}
			else if ( str.size == 11 )
...
// 	wait( 0.05 );
	...
	if ( guy.loops > 1 )
	{
		PrintLn( "guy with name " + guy.animname + " has " + guy.loops + " looping animations played: " );
		for ( i = 0; i < guy.loopAnims.size; i++ )
			PrintLn( "anim: " + guy.loopAnims[ i ] );
		AssertMsg( "Multiple looping animations playing on guy, see console output." );
	}
}

looping_anim_ender( guy, ender, anime )
{
	guy endon( "death" );
	self waittill( ender );
...
=============
///ScriptDocBegin
"Name: removeNotetrack( <animname>, <notetrack> , <anime> , <noteType> )"
"Summary: Removes a notetrack triggered event"
"Module: Anim"
"MandatoryArg: <animname>: Animname of the scene "
"MandatoryArg: <notetrack>: notetrack "
"MandatoryArg: <anime>: The scene "
"MandatoryArg: <noteType>: The type of notetrack event added ( "sound", "attach model", "function", "detach model", "dialog", "flag", "flag_clear", "notify", "effect", "stop_effect" ) "
"OptionalArg: <ent>: The contents of the notetrack event you want to remove. Usefull if there are two similar events (like two dialogs or functions), this adds another check so that you only remove the one you want"
"Example: 	removeNotetrack( "price", "smoke_cigar", "cool_melee_kill", "function" ); "
"SPMP: singleplayer"
///ScriptDocEnd
=============
*/
removeNotetrack( animname, notetrack, anime, noteType, ent )
{
	...
	// check if notetrack has any events set
	if ( !IsDefined( array ) || !IsArray( array ) || array.size < 1 )
		return;

	// look for notetrack event
	...
	// check if a noteType event was found
	if ( index < 0 )
		return;
	
	// remove notetrack event
	if ( array.size == 1 )
		array = [];
	else
...
=============
///ScriptDocBegin
"Name: addNotetrack_playersound( <animname> , <notetrack> , <scene> , <soundalias> )"
"Summary: Makes a sound play on the player when a certain notetrack is hit. "
"Module: Anim"
"MandatoryArg: <animname>: The animname of the character, or generic. "
"MandatoryArg: <notetrack>: The notetrack in the anim, usual dialog. "
"MandatoryArg: <scene>: The scene the sound plays in. "
...
=============
///ScriptDocBegin
"Name: addNotetrack_notify( <animname> , <notetrack> , <theNotify> , <anime> )"
"Summary: Associates a notetrack to send a notify to the level."
"Module: Anim"
"CallOn: An entity"
"MandatoryArg: <animname>: "
"MandatoryArg: <notetrack>: "
"MandatoryArg: <theNotify>: "
"MandatoryArg: <anime>: The animation scene as defined in the map_anim.gsc file "
"Example:  addNotetrack_notify( "van_mate" , "dialog" , "end_get_in_the_van" , "end_get_in" );"
"SPMP: singleplayer"
///ScriptDocEnd
=============
*/
