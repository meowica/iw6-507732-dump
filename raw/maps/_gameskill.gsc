...
display_screen_effect( type, side, material, extra_bottom_material, scaler )
{
	...
	switch( side )
	{
		case "bottom":
			...
			{
				// Flash of dirt
				hud = create_client_overlay_custom_size( material, 1 );
				hud thread screen_effect_on_open_bottom( type, scaler, true );
				hud screen_detailed_alpha();
			}
			else // bloodsplat
			{
				hud = create_client_overlay_custom_size( material, 0 );
			...
			{
				// Fullscreen dirt
				detailed = create_client_overlay_custom_size( extra_bottom_material, 0 );
...
playerHealthRegen()
{
	...
	for ( ;; )
	{
		...
		if ( laststand_enabled() )
		{
			AssertEx( IsDefined( level.laststand_player_func ), "Laststand enabled without level.laststand_player_func initialized." );
...
playerBreathingSound( healthcap )
{
	...
	for ( ;; )
	{
		...
		// JS: added breathing_time for underwater breathing
		breathing_time = 0.1;
		if (isdefined(level.player.gs.custombreathingtime))
			breathing_time = level.player.gs.custombreathingtime;
...
init_take_cover_warnings()
{
	...
	isPreGameplayLevel = IsDefined( level.isPreGameplayLevel ) && level.isPreGameplayLevel;
