...
{
	if ( moveMode == "sprint" )
		self SetAnimState( moveMode, 0, 1.6 );		// HACK: speeding up the movement until we get the sprint animations, 1.6 is from 24 run speed divided by 36 sprint speed
	else
...
DoSharpTurn( newDir )
{
	...
	self PlayAnimNUntilNotetrack( animState, angleIndex, "sharp_turn", "code_move" );
...
WaitForStop()
{
	...
	self PlayAnimNUntilNotetrack( stopState.state, stopState.index, "move_stop", "end" );
...
StartMove()
{
	...
	self PlayAnimNUntilNotetrack( "move_start", angleIndex, "move_start", "code_move" );
