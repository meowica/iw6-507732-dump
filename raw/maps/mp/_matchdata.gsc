...
logLoadout( lifeId )
{
	...
	else if( isSubstr( class, "custom" ) )
	{
		...
		loadoutOffhand = "specialty_null"; //maps\mp\gametypes\_class::cac_getOffhand( class_num );

		loadoutEquipment = maps\mp\gametypes\_class::cac_getPerk( class_num, 0 );
		loadoutPerk1 = "specialty_null"; //maps\mp\gametypes\_class::cac_getPerk( class_num, 1 );
		loadoutPerk2 = "specialty_null"; //maps\mp\gametypes\_class::cac_getPerk( class_num, 2 );
		loadoutPerk3 = "specialty_null"; //maps\mp\gametypes\_class::cac_getPerk( class_num, 3 );
		loadoutStreakType = "specialty_null"; //maps\mp\gametypes\_class::cac_getPerk( class_num, 5 );
		loadoutActiveAbility = "specialty_null"; //maps\mp\gametypes\_class::cac_getActiveAbility( class_num );
		loadoutPassiveAbility = "specialty_null"; //maps\mp\gametypes\_class::cac_getPassiveAbility( class_num );
		loadoutPrimaryBuff = "specialty_null"; //maps\mp\gametypes\_class::cac_getWeaponBuff( class_num, 0 );
		loadoutSecondaryBuff = "specialty_null"; //maps\mp\gametypes\_class::cac_getWeaponBuff( class_num, 1 );
		loadoutKillstreak1 = "none"; //maps\mp\gametypes\_class::cac_getKillstreak( class_num, loadoutStreakType, 0 );
		loadoutKillstreak2 = "none"; //maps\mp\gametypes\_class::cac_getKillstreak( class_num, loadoutStreakType, 1 );
		loadoutKillstreak3 = "none"; //maps\mp\gametypes\_class::cac_getKillstreak( class_num, loadoutStreakType, 2 );
	}
	else
	{
		class_num = getClassIndex( class );
		...
		loadoutActiveAbility = maps\mp\gametypes\_class::table_getActiveAbility( level.classTableName, class_num );
		loadoutPassiveAbility = maps\mp\gametypes\_class::table_getPassiveAbility( level.classTableName, class_num );
	...
	//setMatchData( "lives", lifeId, "activeAbility", loadoutActiveAbility );
	//setMatchData( "lives", lifeId, "passiveAbility", loadoutPassiveAbility );
	...
	//	setMatchData( "lives", lifeId, "supportStreaks", 0, loadoutKillstreak1 );
	//	setMatchData( "lives", lifeId, "supportStreaks", 1, loadoutKillstreak2 );
	//	setMatchData( "lives", lifeId, "supportStreaks", 2, loadoutKillstreak3 );
	//	break;
...
{	
	...
	if ( isDefined( sWeaponType ) && (sWeaponType == "primary" || sWeaponType == "altmode") && (sWeaponClass == "pistol" || sWeaponClass == "smg" || sWeaponClass == "rifle" || sWeaponClass == "spread" || sWeaponClass == "mg" || sWeaponClass == "grenade" || sWeaponClass == "rocketlauncher" || sWeaponClass == "sniper") )
	{
		...
		if ( weaponTokens[0] == "iw5" || weaponTokens[0] == "iw6" )
		{
			...
			if ( isAttachmentSniperScopeDefaultTokenized( weaponTokens, weaponTokens[ 3 ] ) )
			{
				weaponTokens[ 3 ] = undefined;
			}
			else if ( isAttachmentSniperScopeDefaultTokenized( weaponTokens, weaponTokens[ 4 ] ) )
			{
...
endOfGameSummaryLogger()
{
	...
	{	
		...
		if ( isDefined( player.detectedExploit ) && player.detectedExploit )
			player setPlayerData( "restXPGoal", player.detectedExploit );				
