...
checkToTurnOffEmp()
{
	...
	if ( (level.teamBased && ( level.teamEMPed[self.team] || level.teamNukeEMPed[self.team] ) || (!level.teamBased && isDefined( level.empPlayer ) && level.empPlayer != self) 
	|| (!level.teamBased && isDefined( level.nukeInfo.player ) && level.nukeInfo.player != self) ) )
			return;
