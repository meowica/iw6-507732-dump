...
_beginLocationSelection ( streakName, selectorType, directionality , size )
{
	...
	if ( IsDefined( streakName) && self.team != "spectator" )
	{
		...
		{
			self.streakMsg = self maps\mp\gametypes\_hud_util::createFontString( "hudbig", .65 );
...
leaderDialog( dialog, team, group, excludeList, location )
{
	...
	if ( level.splitscreen )
	{
		if ( level.players.size )
			level.players[ 0 ] leaderDialogOnPlayer( dialog, group, undefined, location );
		return;
	}
	...
	if ( IsDefined( group ) )
	{
		...
		if ( self.leaderDialogGroup == group )
		{
			if ( groupOverride )
			{
				...
				self thread playLeaderDialogOnPlayer( dialog, team, location );
			}
...
playLeaderDialogOnPlayer( dialog, team, location )
{
	self endon( "disconnect" );
	...
	if ( self.leaderDialogQueue.size > 0 )
	{
		...
		self thread playLeaderDialogOnPlayer( nextDialog, team, nextLoc );
	}
}
...
============= 
*/
givePerk( perkName, useSlot, perkPower )
{
	...
	self _setPerk( perkName, useSlot, perkPower );
...
givePerkEquipment( perkName, useSlot )
{
	...
	if( IsSubStr( perkName, "_mp" ) )
	{
		...
		if ( self _hasPerk( "specialty_extra_deadly" ) )
			self SetWeaponAmmoClip( perkName, 2 );
		else
...
givePerkOffhand( perkName, useSlot )
{
	...
	if( IsSubStr( perkName, "_mp" ) )
	{
		...
		case "emp_grenade_mp":
		case "motion_sensor_mp":
			if ( self _hasPerk( "specialty_extra_equipment" ) )
				self SetWeaponAmmoClip( perkName, 2 );
			else
...
_setPerk( perkName, useSlot, perkPower )
{
	...
	self.perksPerkPower[ perkName ] = perkPower;
	
	if ( IsDefined( level.perkSetFuncs[perkName] ) )
	{
		if ( isDefined(perkPower) )
			self thread [[ level.perkSetFuncs[perkName] ]]( perkPower );
		else
	...
	if ( IsDefined( level.killstreakChainingWeapons[weapon] ) )	
	{
		...
		for( i = KILLSTREAK_SLOT_1; i < KILLSTREAK_SLOT_3 + 1; i++ )
		{
			...
			if( IsDefined( self_pers_killstreaks[i] ) && 
				...
				self_pers_killstreaks[i].streakName == level.killstreakChainingWeapons[weapon] && 
...
isKillstreakWeapon( weapon )
{
	...
	if( IsDefined( WeaponInventoryType( weapon ) ) && 
	   	WeaponInventoryType( weapon ) == "exclusive" )
		return true;
...
getWeaponAttachmentArrayFromStats( weaponName )
{
	...
	for ( i = 11; i <= 21; i++ )
	{
		attachment = tablelookup( "mp/statstable.csv", 4, weaponName, i );
		if ( attachment == "" )
			break;
...
attachmentMap( attachmentName, weaponName )
{
	...
	if ( !IsDefined( level.attachmentMap_baseToUnique ) )
	{
		buildAttachmentMaps();
	}
...
{
	if ( !IsDefined( level.attachmentMap_uniqueToBase ) )
	{
		buildAttachmentMaps();
	}
...
validateUseStreak() // self == player
{
	...
	if( isDeadlyKillstreak( streakName ) && level.killstreakRoundDelay && getGametypeNumLives() )
	{
...
============= 
///ScriptDocBegin
"Name: isDeadlyKillstreak( <streakName> )"
"Summary:  Returns true or false if the killstreak is one that can kill other players."
"Module: Utility"
"CallOn: None"
"MandatoryArg: <streakName> The killstreak name to check against."
"Example: if( isDeadlyKillstreak( "predator_missile" ) )"
"SPMP: multiplayer"
///ScriptDocEnd
============= 
*/
isDeadlyKillstreak( streakName )
{
	switch ( streakName )
	{
	case "predator_missile":
...
lightWeightScalar( power )
{
	...
	//this is an additional scalar ranges from 1-10 (so lightweight will return 101-110 given a 1.10 scalar)
	power = Int(power);
	val = LIGHTWEIGHT_SCALAR - 1;
	realScalar = ((power * val)/10) + 1;
...
GetMatchRulesSpecialClass( team, index )
{
	...
	class["loadoutActiveAbility"] = getMatchRulesData( "defaultClasses", team, index, "class", "activeAbility" );	
...
updateSessionState( sessionState, statusIcon )
{
	...
	self SetClientDvar( "ui_session_state", GetIndexForLuiNCString( sessionState ) );
}
...
isSpecialistKillstreak( refString )
{
	...
	case "specialty_bulletpenetration_ks":
...
=============
///ScriptDocBegin
"Name: isTacticalKillstreak( <killstreak name> )"
"Summary: Returns true/false if the killstreak is a Tactical killstreak."
"CallOn: none"
"MandatoryArg: <killstreak name> The killstreak name to check out."
"Example: if( isTacticalKillstreak( "tactical_assault_1" ) )"
///ScriptDocEnd
============
*/
isTacticalKillstreak( refString )
{
	return ( isTacticalAssaultKillstreak( refString ) || isTacticalSupportKillstreak( refString ) );
}

/* 
=============
///ScriptDocBegin
"Name: isTacticalAssaultKillstreak( <killstreak name> )"
"Summary: Returns true/false if the killstreak is a Tactical Assault killstreak."
"CallOn: none"
"MandatoryArg: <killstreak name> The killstreak name to check out."
"Example: if( isTacticalAssaultKillstreak( "tactical_assault_1" ) )"
///ScriptDocEnd
============
*/
isTacticalAssaultKillstreak( refString )
{
	switch ( refString )
	{
	case "tactical_assault_1":
	case "tactical_assault_2":
	case "tactical_assault_3":
	case "tactical_assault_4":
	case "tactical_assault_5":
	case "tactical_assault_6":
	case "tactical_assault_7":
	case "tactical_assault_8":
	case "tactical_assault_9":
		return true;
	default:
		return false;
	}
...
=============
///ScriptDocBegin
"Name: isTacticalSupportKillstreak( <killstreak name> )"
"Summary: Returns true/false if the killstreak is a Tactical Support killstreak."
"CallOn: none"
"MandatoryArg: <killstreak name> The killstreak name to check out."
"Example: if( isTacticalSupportKillstreak( "tactical_assault_1" ) )"
///ScriptDocEnd
============
*/
isTacticalSupportKillstreak( refString )
{
	switch ( refString )
	{
	case "tactical_support_1":
	case "tactical_support_2":
	case "tactical_support_3":
	case "tactical_support_4":
	case "tactical_support_5":
	case "tactical_support_6":
	case "tactical_support_7":
	case "tactical_support_8":
		return true;
	default:
		return false;
	}
...
"Summary: Returns the array value of the helicopter pilot mesh offset."
"CallOn: none"
