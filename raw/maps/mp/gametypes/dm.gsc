...
onSpawnFinished()
{
	...
	// are we playing the playlist recipe for "all or nothing"?
	if ( matchMakingGame() && self.secondaryWeapon == "none" )
	{		
		//	no drops
		if ( !isDefined( level.blockWeaponDrops ) || !level.blockWeaponDrops )
			level.blockWeaponDrops = true;
			
		if ( !isDefined( level.aon_createdKillTriggers ) )
		{
			level.aon_createdKillTriggers = true;
