...
getPerferedDomPoints( teamDomPoints, enemyDomPoints )
{
	...
	if( teamDomPoints.size == level.domFlags.size )
	{
		...
		bestFlagPoint 	= level.bestSpawnFlag[ self.pers["team"] ];
	...
	if( (teamDomPoints.size == 1) && (enemyDomPoints.size == 2) )
	{
...
onBeginUse( player )
{
	...
	if( self.label == "_a" )
		...
		domFlagIdx = 2;
	else if( self.label == "_c" )
		domFlagIdx = 3;
	
	player SetClientOmnvar( "ui_dom_securing", domFlagIdx );
...
onUseUpdate( team, progress, change )
{
	...
	if ( progress > 0.05 && change && !self.didStatusNotify )
	{
		...
		self maps\mp\gametypes\_gameobjects::set2DIcon( "enemy", "waypoint_contested" + self.label );
		self maps\mp\gametypes\_gameobjects::set3DIcon( "enemy", "waypoint_contested" + self.label );
		self maps\mp\gametypes\_gameobjects::set2DIcon( "friendly", "waypoint_contested_kill" + self.label );
		self maps\mp\gametypes\_gameobjects::set3DIcon( "friendly", "waypoint_contested_kill" + self.label );

		self.didStatusNotify = true;
	}
	else if( level.allowNeutral && (progress > 0.49) && change && self.didStatusNotify && (ownerTeam != "neutral") )
	{
...
giveFlagCaptureXP( touchList )
{
	...
	for ( index = 0; index < players.size; index++ )
	{
		...
		player.extrascore0 = player.pers[ "captures" ];
...
updateDomScores()
{
	...
	while ( !level.gameEnded )
	{
		...
		if ( matchMakingGame() && !level.allowNeutral && (domFlags.size < 2) && (getTimePassed() > 120000) )
		{
