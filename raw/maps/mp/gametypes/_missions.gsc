...
ch_hardpoints( data )
{
	...
	case "directional_uav":
		player processChallenge( "ch_directional_uav" );
		player processChallenge( "ch_support_streaks" );
		//if ( player.pers["directional_uav"] >= 3 )
		//	player processChallenge( "ch_???" );
		break;
...
ch_kills( data, time )
{
	...
	// this needs to be processed before we filter out for killstreak weapons because you could the final kill with a killstreak weapon
	if ( isDefined( player.finalKill ) )
		...
		else
		{
			...
			switch ( weaponClass )
			{
				...
				case "weapon_shotgun":
					...
					break;
				case "weapon_dmr":
//					player processChallenge( "ch_dmr_kill" );	// ToDo: Update with challenge pass
					break;
				case "weapon_sniper":
				...
				case "weapon_machine_pistol":
					...
					break;
				case "weapon_lmg":
					player processChallenge( "ch_lmg_kill" );
					break;
				default:
			}
	...
	else if ( data.sMeansOfDeath == "MOD_HEAD_SHOT" )
	{
		...
		//daily weapon challenge tracking for headshot kills.
		switch ( weaponClass )
		{
			case "weapon_smg":
				player processChallenge( "ch_smg_kill" );
			...
			case "weapon_shotgun":
				...
				break;
			case "weapon_dmr":
//				player processChallenge( "ch_dmr_kill" );
				break;
			case "weapon_sniper":
				player processChallenge( "ch_sniper_kill" );
				break;
...
masteryChallengeProcess( baseWeapon )
{
	...
	{
		attachmentName = getWeaponAttachmentFromStats( baseWeapon, index );
...
updateChallenges()
{
	...
	foreach ( challengeRef, levelChallengeData in level.challengeInfo )
	{
		...
		// The order of operations here is significant. IsItemUnlocked is *very* expensive and so things are arranged to keep the number of calls
		// to IsItemUnlocked to a minimum. updateChallenges must be sliced across multiple frames (see the challengeCount code above) to avoid
		// stalling the server when clients join. TableLookup is also generally a slow function (though not as bad as IsItemUnlocked) and should
		// be avoided here. TableLookupByRow is best. Be mindful of this when adding new challenges and types of challenges to this processing loop.
		// The best strategy is usually to do the heavy processing of challenge information in buildChallengeTableInfo (below) and store the
		// information in level.challengeInfo. This is done for example with weapon and killstreak challenges.
		...
		else
		{
			...
			if ( IsDefined( levelChallengeData["weapon"] ) )
			{
				if ( !self isChallengeUnlocked( challengeRef, challengeIndex ) )		// make sure the challenge is unlocked
					continue;

				baseWeapon = levelChallengeData["weapon"];
				if( !self IsItemUnlocked( baseWeapon ) )								// make sure the weapon is unlocked
					continue;

				if ( IsDefined( levelChallengeData["attachment"] ) )
				{
					if( !maps\mp\gametypes\_class::isAttachmentUnlocked( baseWeapon, levelChallengeData["attachment"] ) )
						continue;
				}
			}
			else if( IsDefined( levelChallengeData["killstreak"] ) )
			{
				if ( !self isChallengeUnlocked( challengeRef, challengeIndex ) )		// make sure the challenge is unlocked
					continue;

				killstreakName = levelChallengeData["killstreak"];
				if( IsDefined( killstreakName ) )
				{
					if( !self GetCommonPlayerData( "killstreakUnlocked", killstreakName ) )	// make sure the killstreak is unlocked
						continue;
				}
			}
			else
...
buildChallengeTableInfo( tableName, typeId )
{
	...
	for ( index = 1; refString != ""; index++ )
	{
		...
		else if( isKillstreakChallenge( refString ) ) 	// make sure the killstreak is unlocked
		{
...
genericChallenge( challengeType, value )
{
	switch ( challengeType )
	{
		...
		case "weapon_lmg":
			...
			break;
		case "weapon_dmr":
//			self processChallenge( "ch_dmr_sniper" );
			break;
		case "weapon_sniper":
