...
handleNormalDeath( lifeId, attacker, eInflictor, sWeapon, sMeansOfDeath )
{
	...
	killCreditTo incPersStat( "kills", 1 );
	...
	{
		if ( sMeansOfDeath == "MOD_MELEE" || attacker killShouldAddToKillstreak( sWeapon ) )
		{
		...
		foreach ( player in self.attackers )
		{
			...
			if( player _hasPerk( "specialty_assists" ) )
			{
				...
				if( !( player.pers["assistsToKill"] % 2 ) )
				{
					...
					if( !isKillstreakWeapon( sWeapon ) && player.pers["cur_kill_streak_for_nuke"] == numKills )
					{
						player thread maps\mp\killstreaks\_killstreaks::giveKillstreak( "nuke", false, true, player );
...
#/	
	...
	// see if the attacker is going to absorb the victim's ability
	if( IsDefined( attacker ) && IsDefined( victim ) && IsPlayer( attacker ) && IsPlayer( victim ) && attacker != victim )
		attacker thread maps\mp\abilities\_abilities::absorbAbility( victim );

	// see if the attacker has the fastRecharge ability
	if( IsDefined( attacker ) && IsDefined( victim ) && IsPlayer( attacker ) && IsPlayer( victim ) && attacker != victim )
	{
		if( IsDefined( attacker.pers[ "abilityRecharging" ] ) && attacker.pers[ "abilityRecharging" ] )
	...
	if( IsDefined( attacker ) && IsDefined( victim ) && IsPlayer( attacker ) && IsPlayer( victim ) && attacker != victim )
	{
		if( IsDefined( attacker.pers[ "abilityOn" ] ) && attacker.pers[ "abilityOn" ] )
	...
	victim maps\mp\gametypes\_weapons::dropWeaponForDeath( attacker, sMeansOfDeath );
...
#/
	...
	{
		recordFinalKillCam( 5.0, victim, attacker, attackerNum, killcamentityindex, killcamentitystarttime, sWeapon, deathTimeOffset, psOffsetTime, sMeansOfDeath );
	}
	...
	if ( !(IsDefined( victim.cancelKillcam) && victim.cancelKillcam) && doKillcam && level.killcam && game[ "state" ] == "playing" && !victim isUsingRemote() && !level.showingFinalKillcam )
	{
		...
		victim maps\mp\gametypes\_killcam::killcam( attackerNum, killcamentityindex, killcamentitystarttime, sWeapon, postDeathDelay + deathTimeOffset, psOffsetTime, timeUntilSpawn, maps\mp\gametypes\_gamelogic::timeUntilRoundEnd(), attacker, victim, sMeansOfDeath );
	}
...
recordFinalKillCam( delay, victim, attacker, attackerNum, killCamEntityIndex, killCamEntityStartTime, sWeapon, deathTimeOffset, psOffsetTime, sMeansOfDeath )
{
...
doFinalKillcam()
{
	...
	if ( IsDefined( attacker ) )
	{
		...
		// since the final kill and the last dog tag pick up can be at different times, we need to process these final kill challenges here for kill confirmed
		if( level.gametype == "conf" && IsDefined( level.finalKillCam_attacker[ attacker.team ] ) && level.finalKillCam_attacker[ attacker.team ] == attacker )
		{
			...
			case "pavelow_minigun_mp":		// Pave Low
				attacker maps\mp\gametypes\_missions::processChallenge( "ch_transformer" );
				break;
			...
			case "cobra_20mm_mp":			// Attack Helicopter
				attacker maps\mp\gametypes\_missions::processChallenge( "ch_og" );
				break;
	...
	foreach ( player in level.players )
	{
		...
		player thread maps\mp\gametypes\_killcam::killcam( attackerNum, killcamentityindex, killcamentitystarttime, sWeapon, postDeathDelay + deathTimeOffset, psOffsetTime, 0, 12, attacker, victim, sMeansOfDeath );
	}
...
getKillcamEntity( attacker, eInflictor, sWeapon ) // self == victim
{
	...
	case "agent_mp":					// squad member
	case "guard_dog_mp":				// guard dog
	case "gas_strike_mp":				// gas airstrike
		return eInflictor.killCamEnt;

	case "cobra_ffar_mp":				// heli missile	
		if ( isDefined( eInflictor.owner ) )
			return eInflictor.owner;
...
{	
	...
	if ( sWeapon == "xm45_mp" && sMeansOfDeath != "MOD_PROJECTILE" )
	{
...
#/
	...
	if ( !(iDFlags & level.iDFLAGS_NO_PROTECTION) )
	{
		...
		// extra damage to cranked players
		if( GetDvar( "g_gametype" ) == "cranked" )
		{
			if( IsDefined( victim.crankedItem ) )
		...
		if ( IsDefined( damager) && damager != victim && iDamage > 0 && ( !IsDefined( sHitLoc ) || sHitLoc != "shield" ) )
		{
			if( iDFlags & level.iDFLAGS_STUN )
				...
				typeHit = "hitendgame";
			else if( IsDefined( victim.lightArmorHP ) && sMeansOfDeath != "MOD_HEAD_SHOT" )
...
#/
	...
	if ( IsAgent( self ) )
	{
		assert( IsDefined( level.agent_funcs ) && IsDefined( level.agent_funcs[self.agent_type]["on_damaged_finished"] ) );
		self [[ level.agent_funcs[self.agent_type]["on_damaged_finished"] ]]( eInflictor, eAttacker, iDamage, iDFlags, sMeansOfDeath, sWeapon, vPoint, vDir, sHitLoc, psOffsetTime );
	}	
...
#/
	...
	if ( ( (self isUsingRemote() ) && (iDamage >= self.health) && !(iDFlags & level.iDFLAGS_STUN) ) || self isRocketCorpse() )
	{
		if ( !IsDefined( vDir ) )
			vDir = ( 0,0,0 );
