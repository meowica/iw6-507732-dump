...
updateDevSettings()
{
	...
	if ( dvar_scr_givekillstreak != "" )
	{
		...
		if( streakName == "juggernaut" || streakName == "juggernaut_recon" || streakName == "juggernaut_maniac" )
		{
	...
	if ( GetDvar( "scr_spam_splashes" ) != "" )
	{
		foreach( player in level.players )
		{
			...
			player thread maps\mp\gametypes\_hud_message::killstreakSplashNotify( "precision_airstrike", 5 );
		...
		if( clearDomFlags )
		{
			if( level.gameType == "dom" )
			{
				foreach( domFlag in level.domFlags )
				{
					...
					domFlag.visuals[0] SetModel( game[ "flagmodels" ][ "neutral" ] );
				}
			}
		}
	...
	if( GetDvarInt( "scr_devIntel" ) > 0 )
	{
		...
		case 2: // bot target player
			// first take it away from who has it
			if( IsDefined( level.intelEnt ) && IsDefined( level.intelEnt[ "owner" ] ) )
			{
				intelOwner = level.intelEnt[ "owner" ];
				if( IsPlayer( intelOwner ) )
					intelOwner thread maps\mp\gametypes\_damage::finishPlayerDamageWrapper( intelOwner, intelOwner, 999999, 0, "MOD_EXPLOSIVE", "nuke_mp", intelOwner.origin, intelOwner.origin, "none", 0, 0 );
			}

			wait( 1.0 );

			// now give it to a bot
			foreach( player in level.players )
			{
				if( IsBot( player ) && IsAlive( player ) )
				{
					...
					SetDevDvar( "scr_devIntelChallengeNum", 2 );
					
					// put it on them so they pick it up
					if( !IsDefined( level.intelEnt ) )
						player maps\mp\gametypes\_intel::spawnFirstIntel();
					else
					{
						newOrigin = GetGroundPosition( player.origin + ( 0, 0, 32 ), 42 );

						level.intelEnt["visuals"].origin = newOrigin;
		...
		case 3: // clear the intel challenge num
			SetDevDvar( "scr_devIntelChallengeNum", -1 );
			break;
		}
	...
	if( GetDvarInt( "scr_devKillDog" ) > 0 )
	{
		...
		{
			if( IsDefined( agent.agent_type ) && agent.agent_type == "dog" )
				agent maps\mp\agents\dog\_dog_think::killDog();
		}
...
//		kick( index );
	...
	{	
		...
		level notify( "bot_connect_monitor" ); // terminate the bot connect monitor immediately

		if ( fullBots  )
		{		
			[[ level.bot_funcs["bots_spawn"] ]]( 1, "autoassign" );
		}
		else
		{
            ent[i] = addtestclient( 1 );
...
showSpawnpoint( spawnpoint, classname, color )
{
	...
	print3d(spawnpoint.origin + (0, 0, 72), classname, color, 1, 1);
	#/
}
...
showSpawnpoints()
{
	...
	if( isdefined( level.spawnpoints ) )
	{
		foreach( spawnpoint in level.spawnpoints )
		{
			...
			if( IsDefined(spawnpoint.sights[hostTeam]) && spawnpoint.sights[hostTeam] != 0 )
			{
				color = (0,1,0);
			}
	...
	if( isdefined( level.startSpawnPoints ) )
	{
		...
		{
			showSpawnpoint( spawnpoint, spawnpoint.classname, (0,0,1) );
		}
	}
