...
main()
{
	...
	level.sr_dogtag_3d_icons = false;
	...
	SetDvar( "ui_bomb_timer_endtime", -1 );
...
onSpawnPlayer()
{
	...
	self SetClientDvar( "ui_bomb_icon_name", "hud_suitcase_bomb" );	
	...
	if ( self.sessionteam == "axis" || self.sessionteam == "allies" )
	{
		...
		if( IsDefined( self.pers[ "denied" ] ) )
			self.extrascore0 = self.pers[ "denied" ];
...
onPlayerKilled(eInflictor, attacker, iDamage, sMeansOfDeath, sWeapon, vDir, sHitLoc, psOffsetTime, deathAnimDuration, killId)
{
	...
	//FIXME: not if this is the last living player on the victim's team - that's game over
	level thread spawnDogTags( self, attacker );
	//FIXME: should be a splash and localized
...
bombPlanted( destroyedObj, player )
{
	...
	*/
	...
	else
	{
		...
		explosionEffect = spawnFx( level._effect[effect], explosionOrigin + (0,0,50), (0,0,1), (cos(rot),sin(rot),0) );
...
spawnDogTags( victim, attacker )
{
	...
	else
	{
		...
		//	we don't need these
		//FIXME: commented this out because these are apparently not created anymore?
		...
		level.dogtags[victim.guid] sr_dogtag_broadcast(victim.team, false, pos );
		level.dogtags[victim.guid] sr_dogtag_broadcast(enemy_team, true, pos );
	...
	objective_position( level.dogtags[victim.guid].objId[victim.team], pos );
	objective_state( level.dogtags[victim.guid].objId[victim.team], "active" );
	Objective_Team( level.dogtags[victim.guid].objId[victim.team], victim.team );
	
	objective_position( level.dogtags[victim.guid].objId[enemy_team], pos );
	objective_state( level.dogtags[victim.guid].objId[enemy_team], "active" );
	Objective_Team( level.dogtags[victim.guid].objId[enemy_team], enemy_team );
//	objective_player( level.dogtags[victim.guid].objId, attacker getEntityNumber() );		
	...
	victim thread maps\mp\gametypes\_hud_message::SplashNotify( "sr_down" );
	level notify( "sr_player_killed", victim );
	sr_splashNotifyTeam( "sr_ally_down", level.dogtags[victim.guid].victimTeam, victim );
	//sr_splashNotifyTeam( "sr_enemy_down", enemy_team );
	if ( IsDefined( attacker ) && IsPlayer( attacker ) && attacker.team == enemy_team )
		attacker thread maps\mp\gametypes\_hud_message::SplashNotify("sr_enemy_down");
	//IPrintLnBold( victim.name );
...
sr_dogtag_broadcast(broadcast_team, is_enemy_team, pos)
{
	icon_material = "waypoint_dogtags";
	if ( !is_enemy_team )
	{
		//FIXME: need a red one for your team
		icon_material = "waypoint_dogtags_friendly";
	}
	
	//show a tag on the radar
	objId = maps\mp\gametypes\_gameobjects::getNextObjID();
	self.objId[broadcast_team] = objId;
	objective_add( objId, "active", pos );
	objective_icon( objId, icon_material );
	
	//TODO: splash notify/sound for teams
	
	//3D icon that everyone can see
	if ( level.sr_dogtag_3d_icons )
	{
		dogtagIcon = NewTeamHudElem( broadcast_team );
		self.icon[broadcast_team] = dogtagIcon;
		dogtagIcon setShader( icon_material, 10, 10 );
		dogtagIcon.alpha = 1;//objPoint.baseAlpha;
		dogtagIcon setWayPoint( true, true );
	//	dogtagIcon SetTargetEnt( self.visuals[0] );
		dogtagIcon.x = pos[0];
		dogtagIcon.y = pos[1];
		dogtagIcon.z = pos[2] + 32;
	}

	//TODO: make this a HUD elem on each individual client and fade it in/out depending on line of sight...?
}
...
//FIXME: handle team switches correctly
...
{	
	...
	if ( player.pers["team"] == self.victimTeam )
	{
		...
		// set the extra score value for the scoreboard
		player.extrascore0 = player.pers[ "denied" ];
		...
		sr_splashNotifyTeam( "sr_ally_respawned", self.victimTeam, self.victim );
		sr_splashNotifyTeam( "sr_enemy_respawned", getOtherTeam(self.victimTeam) );
		if ( IsDefined( self.victim ) )
		{
			if ( !level.gameEnded )
				self.victim thread sr_respawn();
			//FIXME: should be a splash and localized
//			IPrintLnBold( self.victim.name );
		}
	...
	//	tell the attacker their kill was denied
		...
		player thread onTagsPickup( event, splash );
	}
	//	enemy pickup
	else
	{
		...
		sr_splashNotifyTeam( "sr_ally_eliminated", self.victimTeam, self.victim );
		sr_splashNotifyTeam( "sr_enemy_eliminated", getOtherTeam(self.victimTeam) );
		if ( IsDefined( self.victim ) )
		{
			//FIXME: should be a splash and localized
//			IPrintLnBold( self.victim.name );
		...
		if ( self.attacker != player )
			self.attacker thread onTagsPickup( event, splash );
		...
		player onTagsPickup( event, splash );
...
resetTags()
{
	...
	objective_state( self.objId[self.victimTeam], "invisible" );
	objective_state( self.objId[getOtherTeam(self.victimTeam)], "invisible" );
	if ( level.sr_dogtag_3d_icons )
	{
		self.icon[self.victimTeam].alpha = 0;
		self.icon[getOtherTeam(self.victimTeam)].alpha = 0;
	}
}
...
clearOnVictimDisconnect( victim )
{
	...
	if ( isDefined( level.dogtags[guid] ) )
	{
		...
		if ( isDefined( level.dogtags[guid] ) )
		{
			...
			objective_delete( level.dogtags[guid].objId["allies"] );
			objective_delete( level.dogtags[guid].objId["axis"] );
