...
onPlayerSpawned()
{
	...
	for(;;)
	{
		...
		{
			self.pers["voiceNum"] = RandomIntRange( 0, 3 );
			
			self.pers["voiceNum"] = 0;
		...
		if ( factionPrefix == "US_" || factionPrefix == "SP_" )
		{
	...
	for( ;; )
	{
		...
		foreach ( grenade in grenades )
		{
			...
			if ( grenadeDistanceSquared < grenande_close_limit_sq )
			{
				...
				{
					if( grenade.model == "projectile_concussion_grenade" )
					{
...
threatCalloutTracking()
{
	...
	while ( 1 )
	{
		...
		{
			if ( isdefined( enemy ) && isReallyAlive( enemy ) && !self _hasPerk( "specialty_coldblooded" ) )
			{
				location = enemy getValidLocation( self );
				found_enemy = true;
				
				if ( isdefined( location ) && self canSay( "callout_location" ) )
				{
...
doSound( soundAlias, hearMyselfSpeak, playDistant )
{
	...
	if ( ( !level.istactical || ( !self _hasPerk( "specialty_coldblooded" ) && self IsSighted() ) ) && level.alivecount[ team ] > 3 && playDistant )
		self thread doSoundDistant( soundalias, team );
	
	if ( isdefined( hearMyselfSpeak ) && hearMyselfSpeak )
	{
...
doThreatCalloutResponse( soundAlias, location )
{
	...
	if ( notify_string == soundAlias )
	{
		foreach( player in level.players )
		{
			...
			if ( DistanceSquared( self.origin, player.origin ) <= CASUALTY_DISTANCE_LIMIT && !isSpeakerInRange( player ) )
			{
				prefix = player.pers["voicePrefix"];
				...
				player thread doSound( newAlias, true, true );
				break;
			}
		}
	}
...
canSay( soundType )
{
	...
	time = GetTime() - level.bcInfo[ "last_say_time" ][ self.pers[ "team" ] ][ soundType ];
	if ( 
	    limit < time 
...
updateChatter( soundType )
{
	...
	level.bcInfo[ "last_say_time" ][ self.pers[ "team" ] ][ soundType ] = GetTime();
	level.bcInfo[ "last_say_pos" ][ self.pers[ "team" ] ][ soundType ] = self.origin;
}
