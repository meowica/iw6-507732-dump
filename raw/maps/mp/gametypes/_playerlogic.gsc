...
spawnPlayer( fauxSpawn )
{
	...
	//this is used for UI so it knows it is not first connect
	self setClientDvar( "ui_options_menu", 0 );
	...
	{
		// TODO: put perk display call here for lua
	}
...
spawnIntermission()
{
	...
	if ( level.rankedMatch && ( self.postGamePromotion || ( IsDefined( self.pers["postGameChallenges"] ) && self.pers["postGameChallenges"] ) ) )
	{
		if ( self.postGamePromotion )
			self playLocalSound( "mp_level_up" );
		else if( IsDefined( self.pers["postGameChallenges"] ) )
			self playLocalSound( "mp_challenge_complete" );

		if ( self.postGamePromotion > level.postGameNotifies )
			level.postGameNotifies = 1;

		if ( IsDefined( self.pers["postGameChallenges"] ) && self.pers["postGameChallenges"] > level.postGameNotifies )
			level.postGameNotifies = self.pers["postGameChallenges"];

		self openMenu( game["menu_endgameupdate"] );

		waitTime = 7.0;
		if( IsDefined( self.pers["postGameChallenges"] ) )
...
Callback_PlayerDisconnect()
{
	...
	foreach( player in level.players )
		player SetClientDvar( "ui_obituary_disconnected", entNum );
...
Callback_PlayerConnect()
{
	...
	if( !level.splitscreen && !isDefined( self.pers["score"] ) )
	{
		entNum = self GetEntityNumber();
		foreach( player in level.players )
			player SetClientDvar( "ui_obituary_connected", entNum );
	}
	...
	if ( game["clientid"] <= 30 && game["clientid"] != getMatchData( "playerCount" ) )
	{
		...
		connectionIDChunkLow = self getCommonPlayerData( "connectionIDChunkLow" );
		connectionIDChunkHigh = self getCommonPlayerData( "connectionIDChunkHigh" );
	...
	////	need to tell the user if they are still on a streak when they connect from the last round they just played, like for S&D
	//cur_kill_streak = self.pers["cur_kill_streak"];
	//switch( cur_kill_streak )
	//{
	//case 0:
	//case 1:
	//case 2:
	...
	//case 3:
	//case 4:
	//	self thread maps\mp\gametypes\_hud_message::splashNotify( "streaking_x2" );
	//	self thread maps\mp\gametypes\_rank::multiplierTextPopup( &"MP_X2_XP" );
	//	break;

	//case 5:
	//case 6:
	//	self thread maps\mp\gametypes\_hud_message::splashNotify( "streaking_x3" );
	//	self thread maps\mp\gametypes\_rank::multiplierTextPopup( &"MP_X3_XP" );
	//	break;

	//case 7:
	//case 8:
	//default:
	//	self thread maps\mp\gametypes\_hud_message::splashNotify( "streaking_x4" );
	//	self thread maps\mp\gametypes\_rank::multiplierTextPopup( &"MP_X4_XP" );
	//	break;
	//}
	...
	else if ( !isDefined( self.pers["team"] ) )
	{
		if ( matchMakingGame() && self.sessionteam != "none" )
		{
			...
			self setClientDvar( "ui_options_menu", 2 );
