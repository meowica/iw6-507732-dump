...
{
	return ( ( ( getTimeLimit() * 60 * 1000 ) - getTimePassed() ) / ( getTimeLimit() * 60 * 1000 ) );
}
...
Callback_StartGameType()
{
	...
	if ( !isDefined( game["gamestarted"] ) )
	{
		...
		//setup multi team faction, for now just use allies default if available other wise use military for all multi-team teams
		if( level.multiTeamBased )
		{
			//NOTE: May want to update getMapCustom to support multi-team team names.
			...
			{
				multiTeamDefaultCharacterSet = "military";
			}
		...
		if( level.multiTeamBased )
		{
			for( i = 0; i < level.teamNameList.size; i++ )
			{
				str_team_eliminated = level.teamNameList[i] + "_eliminated";
				str_team_forfeited = level.teamNameList[i] + "_forfeited";
				str_team_name = level.teamNameList[i] + "_name";
				
				game["strings"][str_team_eliminated] = maps\mp\gametypes\_teams::getTeamEliminatedString( level.teamNameList[i] );
				game["strings"][str_team_forfeited] = maps\mp\gametypes\_teams::getTeamForfeitedString( level.teamNameList[i] );
				game["strings"][str_team_name] = maps\mp\gametypes\_teams::MT_getTeamName( level.teamNameList[i] );
				
				game["icons"][level.teamNameList[i]] = maps\mp\gametypes\_teams::MT_getTeamIcon( level.teamNameList[i] );
				game["colors"][level.teamNameList[i]] = maps\mp\gametypes\_teams::getTeamColor( level.teamNameList[i] );
			}
		}
		...
		if( level.multiTeamBased )
		{
			for( i = 0; i < level.teamNameList.size; i++ )
			{
				if ( game["colors"][level.teamNameList[i]] == (0,0,0) )
				{
					game["colors"][level.teamNameList[i]] = (0.5,0.5,0.5);
				}
			}
		}
	...
	if ( GetDvar( "g_gametype" ) != "aliens" )
	{
		...
		thread maps\mp\gametypes\_intel::init();
...
endGame( winner, endReasonText, nukeDetonated )
{
	...
	foreach ( player in level.players )
	{
		// turning off the end of game summary for now, it's annoying
...
setWeaponStat( name, incValue, statName )
{
	...
	if( isKillstreakWeapon( name ) || weaponClass == "killstreak" || weaponClass == "other" )
		return;
	...
	if( statName != "deaths" && weaponClass != "weapon_projectile" )
	{
		name = self getCurrentWeapon();  
	}
	
	// defensive check to ensure current weapon isnt killstreak weapon
	if( isKillstreakWeapon( name ) || weaponClass == "killstreak" || weaponClass == "other" )
		return; 
	...
	if( statName == "deaths" )
	{
		...
		weaponName = getBaseWeaponName( name );
		...
		*/
		...
		if( tokens[0] != "none" )
		{
			for( i = 0; i < tokens.size; i++ )
			{
				...
				// Handle scope reticles
				if ( GetSubStr( tokens[ i ], 0, 5 ) == "scope" )
					continue;
				
				if ( isAttachmentSniperScopeDefaultTokenized( tokens, tokens[ i ] ) )
					continue;
				
				tokens[i] = validateAttachment( tokens[i] );				
				
				//IW4 weapon check
				if ( i == 0 && tokens[i] != "iw5" && tokens[i] != "iw6" && tokens[i] != "alt" )
					continue;
