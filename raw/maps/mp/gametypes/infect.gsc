...
onStartGameType()
{
	...
	level.infect_timerDisplay setPoint( "TOPLEFT", "TOPLEFT", 200, 5 );
...
gameTimer()
{
	...
	while( true )
	{
		level waittill( "update_game_time" );
...
setInfectedMsg()
{
	if ( isDefined( self.isInitialInfected ) )
	{
		self thread maps\mp\gametypes\_rank::xpEventPopup( &"SPLASHES_FIRST_MERCENARY", (1,0,0), 0.3 );	
	}
	else if ( isDefined( self.changingToRegularInfected ) )
	{
		...
		if ( isDefined( self.changingToRegularInfectedByKill ) )
		{
			...
			self thread maps\mp\gametypes\_rank::giveRankXP( "first_draft" );
		}	
	}
	else
	{
		self thread maps\mp\gametypes\_rank::xpEventPopup( &"SPLASHES_DRAFTED", (1,0,0), 0.3 );	
	}
}
...
chooseFirstInfected()
{
	...
	level.infect_timerDisplay.label = &"MP_DRAFT_STARTS_IN";
	level.infect_timerDisplay setTimer( CONST_FIRST_INFECTED_TIMER );
	level.infect_timerDisplay.alpha = 1;	
	
	maps\mp\gametypes\_hostmigration::waitLongDurationWithHostMigrationPause( CONST_FIRST_INFECTED_TIMER );
	level.infect_timerDisplay.alpha = 0;		
...
onFinalSurvivor()
{
	...
	foreach ( player in level.players )
	{
		if ( player.team == "allies" )
		{
			...
			{
				if ( player.gameModeJoinedAtStart && distance( player.infect_spawnPos, player.origin ) > 32 )
				{
...
{	
	...
	level.infect_loadouts["allies"]["loadoutAbility"] 						= "specialty_null";		
	...
	level.infect_loadouts["axis_initial"]["loadoutAbility"] 				= "specialty_null";	
	level.infect_loadouts["axis_initial"]["loadoutJuggernaut"] 				= false;	
	
	
	//	infected
	level.infect_loadouts["axis"]["loadoutPrimary"] 						= "iw5_knifeonly";
	...
	level.infect_loadouts["axis"]["loadoutAbility"] 						= "specialty_null";		
