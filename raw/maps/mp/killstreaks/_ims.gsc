...
init()
{
	...
	level.imsSettings[ "ims" ].hintString =				&"KILLSTREAKS_IMS_PICKUP";	
	level.imsSettings[ "ims" ].placeString =			&"KILLSTREAKS_IMS_PLACE";	
	level.imsSettings[ "ims" ].cannotPlaceString =		&"KILLSTREAKS_IMS_CANNOT_PLACE";	
	...
	level._effect[ "ims_explode_mp" ]		= LoadFX( "fx/explosions/sentry_gun_explosion" );
	level._effect[ "ims_smoke_mp" ]			= LoadFX( "fx/smoke/car_damage_blacksmoke" );
	level._effect[ "ims_sensor_trail" ]		= LoadFX( "fx/smoke/smoke_geotrail_rpg" );
	level._effect[ "ims_sensor_explode" ]	= LoadFX( "fx/explosions/generator_sparks_d" );
	level._effect[ "ims_antenna_light_mp" ]	= LoadFX( "fx/lights/light_detonator_blink" );
}
...
ims_generic_collision_destroy()
{
	self endon( "carried" );

	self maps\mp\_movers::script_mover_generic_collision_destroy( false );
...
ims_setPlaced() // self == imsForPlayer
{
	...
	ims thread ims_generic_collision_destroy();
...
ims_setCancelled()
{
	if( IsDefined( self.carriedBy ) )
	{
		...
		if( IsDefined( self.carriedBy.imsList ) )
		{
			foreach( ims in self.carriedBy.imsList )
			{
				 if( IsDefined( ims.bombSquadModel ) )
...
ims_setActive() // self == ims
{
	...
	self make_entity_sentient_mp( owner.team );
	if ( IsSentient( self ) )
	{
...
fire_sensor( targetEnt, sensorEnt, lidEnt ) // self == ims
{
	...
	// this keeps it from trying to fire twice
	//	you could pick up the ims before the sensor was done moving and firing and plant it again to get the same sensor to try and fire twice
	sensorEnt.fired = true;
