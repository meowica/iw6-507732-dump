...
doNuke( allowCancel )
{
	...
	level.prevUIBombTimer = GetDvarInt( "ui_bomb_timer" );
...
cancelNukeOnDeath( player )
{
	...
	SetDvar( "ui_bomb_timer", uiBombTimer ); // Nuke sets '4' to avoid briefcase icon showing
    ...
    if( level.multiTeamBased )
    {
        ...
        for( i = 0; i < level.teamNameList.size; i++ )
        {
            if( level.nukeInfo.team != level.teamNameList[i] )
            {
                ...
                level.teamNukeEMPed[level.teamNameList[i]] = true;
            }
        }
    }
