...
init()
{
	...
	level.killstreakFuncs[ "uav_3dping" ] =			::tryUseUAV3DPing;
	level.killstreakFuncs[ "personal_3dping" ] =	::tryUsePersonal3DPing;
	...
	level.uavSettings[ "uav_3dping" ].health =					999999; // keep it from dying anywhere in code	
	level.uavSettings[ "uav_3dping" ].maxHealth =				500; // this is what we check against for death	
	level.uavSettings[ "uav_3dping" ].streakName =				"uav_3dping";
	level.uavSettings[ "uav_3dping" ].modelBase =				"vehicle_skylark";
	level.uavSettings[ "uav_3dping" ].fxId_explode =			LoadFX( "fx/explosions/bouncing_betty_explosion" );	
	level.uavSettings[ "uav_3dping" ].fxId_leave =				undefined;
	level.uavSettings[ "uav_3dping" ].fx_leave_tag =			undefined;
	level.uavSettings[ "uav_3dping" ].fxId_contrail =			undefined;
	level.uavSettings[ "uav_3dping" ].fx_contrail_tag =			undefined;
	level.uavSettings[ "uav_3dping" ].sound_explode =			"ball_drone_explode"; // TODO: get sound
	level.uavSettings[ "uav_3dping" ].teamSplash =				undefined;	
	level.uavSettings[ "uav_3dping" ].calloutDestroyed =		"callout_destroyed_uav";	
	level.uavSettings[ "uav_3dping" ].addFunc =					::addActiveUAV;	
	level.uavSettings[ "uav_3dping" ].removeFunc =				::removeActiveUAV;	
	...
	level.uavSettings[ "uav_3dping" ].fxId_ping =				LoadFX( "fx/ui/3d_world_ping" );
	level.uavSettings[ "uav_3dping" ].sound_ping =				"ball_drone_3Dping";
	level.uavSettings[ "uav_3dping" ].modelHighlightS =			"mp_body_delta_elite_assault_aa_highlight";
	level.uavSettings[ "uav_3dping" ].modelHighlightC =			"mp_body_delta_elite_assault_aa_highlight";
	level.uavSettings[ "uav_3dping" ].modelHighlightP =			"mp_body_delta_elite_assault_aa_highlight";
	
	level.uavSettings[ "personal_3dping" ] = SpawnStruct();
	level.uavSettings[ "personal_3dping" ].streakName =			"personal_3dping";
	level.uavSettings[ "personal_3dping" ].highlightFadeTime =	3.0; // time it takes to fade out the hightlight
	level.uavSettings[ "personal_3dping" ].fxId_ping =			LoadFX( "fx/ui/3d_world_ping" );
	level.uavSettings[ "personal_3dping" ].sound_ping =			"ball_drone_3Dping";
	level.uavSettings[ "personal_3dping" ].modelHighlightS =	"mp_body_delta_elite_assault_aa_highlight";
	level.uavSettings[ "personal_3dping" ].modelHighlightC =	"mp_body_delta_elite_assault_aa_highlight";
	level.uavSettings[ "personal_3dping" ].modelHighlightP =	"mp_body_delta_elite_assault_aa_highlight";
...
/#
	...
	SetDevDvarIfUninitialized( "scr_personal_3dping_highlightFadeTime", level.uavSettings[ "personal_3dping" ].highlightFadeTime );
#/
}
...
tryUseUAV3DPing( lifeId )
{
	return useUAV( "uav_3dping" );
}

tryUsePersonal3DPing( lifeId ) // self == player
{
	uavType = "personal_3dping";

	PlayFX( level.uavSettings[ uavType ].fxId_ping, self.origin );

	self PlayLocalSound( level.uavSettings[ uavType ].sound_ping );

	// highlight all enemies in the world that can't be seen
	foreach( enemy in level.players ) // TODO: level.participants? to include the bots and agents
	{
		...
		if( level.teamBased && enemy.team == self.team )
			continue;
		else if( !level.teamBased && enemy == self )
			continue;

		if( enemy _hasPerk( "specialty_blindeye" ) )
			continue;

		modelHighlight = Spawn( "script_model", enemy.origin );
		modelHighlight.angles = enemy.angles;
		...
		if( !SightTracePassed( self GetEye(), enemy GetEye(), false, self, enemy ) )
			modelHighlight ShowToPlayer( self );
...
/#
		fadeTime = GetDvarFloat( "scr_personal_3dping_highlightFadeTime" );
#/
...
/#
	if( uavType == "uav_3dping" )
		useTime = GetDvarInt( "scr_uav_3dping_timeout", level.uavSettings[ uavType ].timeOut );
	else
...
#/
	...
	if( uavType == "uav_3dping" )
	{
		result = self tryLaunchUAV3DPing( self, useTime, uavType ); // blocking function
		if( !IsDefined( result ) || ( IsDefined( result ) && !result ) ) 
			return false;
	}
	else
...
tryLaunchUAV3DPing( owner, duration, uavType ) // self == player
{	
	self notify( "tryLaunchUAV3DPing" );
	self endon( "tryLaunchUAV3DPing" );

	self endon( "disconnect" );
	level endon( "game_ended" );

	// reworked this to thread all of the functions at once and then watch for what returns
	// this fixes an infinite bug where you can kill the player as they throw it and they'll respawn with another one
	self.threwUAV3DPingMarker = undefined;
	self.threwUAV3DPingMarkerIndex = undefined;
	self thread watchUAV3DPingWeaponChange();
	self thread watchUAV3DPingThrowUsage();
	self thread watchUAV3DPingThrow( owner, duration, uavType );

	result = self waittill_any_return( "notUAV3DPingWeapon", "UAV3DPingThrown" );
	if( IsDefined( result ) && result == "UAV3DPingThrown" )
		return true;
	// result comes back as undefined if the player is killed while throwing, so we need to check to see if they threw the uav before dying
	else if( !IsDefined( result ) && IsDefined( self.threwUAV3DPingMarker ) )
		return true;

	return false;
}

watchUAV3DPingWeaponChange() // self == player
{
	level endon( "game_ended" );

	self notify( "watchUAV3DPingWeaponChange" );
	self endon( "watchUAV3DPingWeaponChange" );

	self endon( "disconnect" );
	self endon( "UAV3DPingThrown" );
	...
	while( currentWeapon == "killstreak_uav_3dping_mp" )
	{
		self waittill( "weapon_change", currentWeapon );
	}

	if( IsDefined( self.threwUAV3DPingMarker ) )
	{
		...
		killstreakWeapon = getKillstreakWeapon( self.pers[ "killstreaks" ][ self.threwUAV3DPingMarkerIndex ].streakName );
		self TakeWeapon( killstreakWeapon );

		self notify( "UAV3DPingThrown" );
	}
	else
		self notify( "notUAV3DPingWeapon" );
}

watchUAV3DPingThrowUsage() // self == player
{
	level endon( "game_ended" );

	self notify( "watchUAV3DPingThrowUsage" );
	self endon( "watchUAV3DPingThrowUsage" );
	...
	while( true )
	{
		...
		if( weaponName != "killstreak_uav_3dping_mp" )
			continue;

		self _disableUsability();

		self beginUAV3DPingTracking();
	}
}

watchUAV3DPingThrow( owner, duration, uavType ) // self == player
{
	level endon( "game_ended" );

	self notify( "watchUAV3DPingThrow" );
	self endon( "watchUAV3DPingThrow" );
	...
	while( true )
	{
		self waittill( "grenade_fire", uav3DPingWeapon, weapname );

		if( weapname != "killstreak_uav_3dping_mp" )
			continue;

		self.threwUAV3DPingMarker = true;
		self.threwUAV3DPingMarkerIndex = self.killstreakIndexWeapon;

		uav3DPingWeapon.owner = self;
		uav3DPingWeapon.weaponName = weapname;

		uav3DPingWeapon thread createUAV3DPing( owner, duration, uavType );	
	}
}

createUAV3DPing( owner, duration, uavType ) // self == uav 3d ping projectile
{
	UAVModel = Spawn( "script_model", self.origin );
...
#/
	...
	UAVModel thread UAV3DPingFlyToPosition( owner, duration, uavType );	

	self delete();
}

UAV3DPingFlyToPosition( owner, duration, uavType ) // self == uavmodel
{
	self endon( "death" );

	team = owner.team;

	self SetModel( level.uavSettings[ uavType ].modelBase );

	self.team = team;
	self.owner = owner;
	self.timeToAdd = 0;
	self.uavType = uavType;
	self.health = level.uavSettings[ uavType ].health;
	self.maxHealth = level.uavSettings[ uavType ].maxHealth;

	self addUAVModel();

	zOffset = RandomIntRange( 3000, 3500 );

	// create a rotating rig and attach this to it, this rig will get cleaned up on uav death
	self.UAVRig = Spawn( "script_model", level.UAVRig.origin );
	self.UAVRig SetModel( "c130_zoomrig" );
	self.UAVRig Hide();
	// used for debug printing
	self.UAVRig.targetname = "uavrig_script_model";

	self thread damageTracker();
	self thread handleIncomingStinger();
	self thread removeUAVModelOnDeath();
	...
	angleVector = VectorNormalize( (xOffset,yOffset,zOffset) );
	angleVector = ( angleVector * RandomIntRange( 500, 1000 ) );

	self.UAVRig.angles = ( 0, sin( angle ), 0 );
	
	// come straight out from the player view
	tempStartPos = owner GetEye() + ( AnglesToForward( owner GetPlayerAngles() ) * 100 );
	tempTargetPos = owner GetEye() + ( AnglesToForward( owner GetPlayerAngles() ) * 500 );
	tempVector = tempTargetPos - owner GetEye();
	tempAngles = VectorToAngles( tempVector );
	self.angles = ( 0, tempAngles[ 1 ], tempAngles[ 2 ] );
	self.origin = tempStartPos;
	self MoveTo( tempTargetPos, 1.0 );
	self waittill( "movedone" );

	// now face the goal position
	tempFinalTargetPos = self.UAVRig.origin + angleVector;
	tempAngles = VectorToAngles( tempFinalTargetPos - tempTargetPos );
	self.angles = tempAngles;
	self MoveTo( tempFinalTargetPos, 3.0 );
	self waittill( "movedone" );

	self LinkTo( self.UAVRig, "tag_origin", angleVector, ( 0, angle - 90, 0 ) );
	self.UAVRig thread rotateUAVRig( 60, "death" );

	self thread updateUAVModelVisibility();	

	self [[ level.uavSettings[ uavType ].addFunc ]]();

	self thread watch3DPing();

	level notify ( "uav_update" );

	self waittill_notify_or_timeout_hostmigration_pause( "death", duration );
	...
	if( self.damageTaken < self.maxHealth )
	{
		...
		if( IsDefined( level.uavSettings[ uavType ].fxId_leave ) && IsDefined( level.uavSettings[ uavType ].fx_leave_tag ) )
			PlayFXOnTag( level.uavSettings[ uavType ].fxId_leave, self, level.uavSettings[ uavType ].fx_leave_tag );

		self waittill_notify_or_timeout_hostmigration_pause( "death", 3 );
		...
		self waittill_notify_or_timeout_hostmigration_pause( "death", 4 + self.timeToAdd );
	}
	...
	while( true )
	{
		...
		foreach( player in level.players )
		{
			if( level.teamBased )
			{
				...
				if( IsDefined( self.uavType ) && self.uavType == "uav_3dping" )
				{
...
beginUAV3DPingTracking()
{
	level endon( "game_ended" );

	self notify( "beginUAV3DPingTracking" );
	self endon( "beginUAV3DPingTracking" );
...
watch3DPing() // self == uav
{
	self endon( "death" );
	level endon( "game_ended" );

	// every N seconds do a ping of the world and show enemies in red
	pingTime = level.uavSettings[ self.uavType ].pingTime;

	while( true )
	{
		//PrintLn( "watch3DPing: GetTime() " + GetTime() + " PingTime " + pingTime );
		PlayFX( level.uavSettings[ self.uavType ].fxId_ping, self.origin );

		if( IsDefined( self.owner ) )
		{
			self.owner PlayLocalSound( level.uavSettings[ self.uavType ].sound_ping );
		}
		...
		foreach( enemy in level.players )
		{
			...
			if( level.teamBased && enemy.team == self.team )
				continue;
			else if( !level.teamBased && enemy == self.owner )
				continue;
			...
			foreach( friendly in level.players )
			{
				if( !isReallyAlive( friendly ) )
					continue;

				if( level.teamBased && friendly.team != self.team )
					continue;
				else if( !level.teamBased && friendly != self.owner )
					continue;

				if( !SightTracePassed( friendly GetEye(), enemy GetEye(), false, friendly, enemy ) )
				{
					modelHighlight ShowToPlayer( friendly );
				
					if ( IsAI( friendly ) )
						friendly ai_3d_sighting_model( modelHighlight, enemy );
				}
			}
			...
			{
			case "stand":
				modelHighlight SetModel( level.uavSettings[ self.uavType ].modelHighlightS );
				break;
			case "crouch":
				modelHighlight SetModel( level.uavSettings[ self.uavType ].modelHighlightC );
				break;
			case "prone":
				modelHighlight SetModel( level.uavSettings[ self.uavType ].modelHighlightP );
				break;
			}

			fadeTime = level.uavSettings[ self.uavType ].highlightFadeTime;
/#
