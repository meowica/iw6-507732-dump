...
givePerksFromKnownLoadout( loadoutPerks )
{
	...
	{
		if ( IsSubStr( perk, "lightweight" ) || IsSubStr( perk, "bulletaccuracy" ) || IsSubStr( perk, "marksman" ) )
		{
			tempArray = StrTok( perk, "_" );
			self givePerk( tempArray[0]+"_"+tempArray[1], false, tempArray[2] );
		}
		else if ( IsSubStr( perk, "specialty_stun_resistance" ) )
		{
			tempArray = StrTok( perk, "_" );
			self givePerk( tempArray[0]+"_"+tempArray[1]+"_"+tempArray[2], false, tempArray[3] );
		}
		else
		{
...
giveHandlingPerks( numPoints )
{
	...
	tempArray = combineArrays(loadoutPerks, self.loadOutPerks);
	self.loadoutPerks = tempArray;
