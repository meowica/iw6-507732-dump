...
patrol( start_target )
{
	...
	// Wait for spawn_function to be called.
	waittillframeend;
	...
	patrol_idle_anim_table["turn180"] = ter_op( IsDefined( self.patrol_anim_turn180 ), self.patrol_anim_turn180, "patrol_turn180" );
	...
	for ( ;; )
	{
		...
		if ( isdefined( currentgoal.script_parameters ) )
		{
			tokens = strtok( currentgoal.script_parameters, " " );
			for ( i=0; i<tokens.size; i++ )
			{
				switch( tokens[i] )
				{
					...
					case "animset":
						i = i+1;
						self.script_animation = tokens[i];
		...
		if ( isdefined( currentgoal.script_moveplaybackrate ) )
		{
			self.moveplaybackrate = currentgoal.script_moveplaybackrate;
		}
		
		if ( 
		    ( isdefined( currentgoal.script_delay ) && currentgoal.script_delay > 0.5 )
		    || isdefined( animtype )
		    || ( IsDefined( currentgoal.script_flag_wait ) && !flag( currentgoal.script_flag_wait ) )
		   )
		{
			if ( !IsDefined( self.patrol_no_stop_transition ) && should_stop )
			{
				// come to a stop
				self patrol_do_stop_transition_anim( animType, reactionAnimThread, currentgoal );
			}
					
			if ( IsDefined( currentgoal.script_flag_wait ) && !flag( currentgoal.script_flag_wait ) )
			{
				flag_wait( currentgoal.script_flag_wait );
			}
			
			currentgoal script_delay();
			
			if ( isdefined( animType ) )
			{
				if ( IsDefined( currentgoal.script_faceangles ) )
				{
					self OrientMode( "face angle", currentgoal.angles[ 1 ] );
				}
				...
				if ( !isdefined( anime ) )
				{
					if ( isdefined( level.patrol_scriptedanims ) )
						anime = level.patrol_scriptedanims[ animType ];
				}
				...
				if ( isdefined( anime ) )
				{
					...
					if ( use_node )
					{
						currentgoal anim_generic_reach( self, anime );
						currentgoal anim_generic_custom_animmode( self, "gravity", anime, undefined, reactionAnimThread );
					}
					else
			...
			if ( 
			    currentgoals.size
			    && ( !isdefined( animtype ) || animType != "turn180" ) 
			    && should_stop
			    && ( !IsDefined( self.skip_start_transition ) || !self.skip_start_transition )
			   )
			{
		...
		{			
			...
			{
				self patrol_do_stop_transition_anim( "path_end_idle", reactionAnimThread, currentgoal );
...
patrol_do_stop_transition_anim( animType, reactionAnimThread, currentgoal )
{
	node = self;
	skip_teleport = false;
	if ( IsDefined( currentgoal.script_faceangles ) )
	{
		node = currentgoal;
		self.noTeleport = true;
		skip_teleport = true;
	}

	if( IsDefined( self.patrol_stop ) && IsDefined( self.patrol_stop[ animType ] ) )
	{
		node anim_generic_custom_animmode( self, "gravity", self.patrol_stop[ animType ], undefined, reactionAnimThread, skip_teleport );
	}
	else if ( IsDefined( self.script_animation ) && isdefined( level.scr_anim[ "generic" ][ "patrol_stop_" + self.script_animation ] ) )
	{
		self anim_generic_custom_animmode( self, "gravity", "patrol_stop_" + self.script_animation, undefined, reactionAnimThread );
	}
	else
	{
		node anim_generic_custom_animmode( self, "gravity", "patrol_stop", undefined, reactionAnimThread, skip_teleport );
	}
}
...
patrol_do_start_transition_anim( animType, reactionAnimThread )
{
	...
	{
		self anim_generic_custom_animmode( self, "gravity", "patrol_start_" + self.script_animation, undefined, reactionAnimThread );
	}
	else
	{
...
set_patrol_run_anim_array()
{
	...
	if ( IsDefined( self.script_animation ) )
	{
		if( isdefined( level.scr_anim[ "generic" ][ "patrol_idle_" + self.script_animation ] ) )
		{
			self set_generic_idle_anim( "patrol_idle_" + self.script_animation );
		}
	}
...
pet_patrol_init_positions( positions )
{
	...
	positions[ "right" ].origin			= origin + ( right * 40 ) + ( forward * 30 );	// right
	positions[ "left" ].origin			= origin + ( right * -40 ) + ( forward * 30 );	// left
	positions[ "back_right" ].origin	= origin + ( right * 32 ) + ( forward * -16 );	// back right
	positions[ "back_left" ].origin		= origin + ( right * -32 ) + ( forward * -16 );	// back left
	positions[ "back" ].origin			= origin + ( forward * -48 );									// back
